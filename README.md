An oligomorphic structure 𝔸 has:
- the _ascending chain property_ (上) over $F$ if $\text{Lin}_F\text{ 𝔸}^d$ is Noetherian for $d = 1, 2, \dots$;
- the _finite length property_ (卡) over $F$ if $\text{Lin}_F\text{ 𝔸}^d$ has finite length for $d = 1, 2, \dots$;
- the property (峠) if it has the finite length property over all fields of characteristic zero.

| class of finite structures | Fraïssé limit | 上 | 卡 | 峠 |
| --- | --- | --- | :--: | --- |
| sets | (ℕ, =) | ✅ | ✅ | ✅ |
| totoal orders | (ℚ, ≤) | ✅ | ✅ | ✅ |
| partial orders | (ℙ, ⊑) |  |  |  |
...
| Boolean algebras | clopens of 2<sup>ℕ</sup> |  |  |  |

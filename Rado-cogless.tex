...
\begin{definition}
    ...
    \begin{enumerate}
        \item \label{item:oligomorphic-approximation-embedding} every finite substructure of $\A$ is a subset of some $\B \in \Bclass$; and
        \item \label{item:oligomorphic-approximation-orbits} there is a common finite upper bound, for all $\B \in \Bclass$, on the number of orbits in $\B^d$ with respect to the automorphisms of $\B$.
    \end{enumerate}
\end{definition}

This is a relaxation of the stronger notion known as \emph{smooth approximation} in model theory. There, the family $\Bclass$ is independent of $d$; 
also, in the place of (2) we require $\B \in \Bclass$ to be homogeneous itself and $\A$ to be oligomorphic~\cite[p.~440]{KLM89}. 
...
For convenience, we summarise these steps in Figure~\ref{fig:proof-summary-cogless}. 
   ...
   %     where the supremum ranges over finite sets of atoms.
    ...
    \lineqfun V W
    \end{align*}
    for the set of all those linear maps from $V$ to $W$ that are equivariant with respect to the action of $G$. (The group and its action are  implicit in this notation.) Elements of this set are closed under taking linear combinations, and therefore the set can be seen as a vector space. (We do not bother to equip this space with an action of $G$ yet --- here it would be trivial.) In particular, it is meaningful to talk about the dimension of this vector space. 

...
\begin{definition}
    ...
    \begin{enumerate}
        \item \label{item:oligomorphic-approximation-embedding} every finite substructure of $\A$ is contained in some $\B \in \Bclass$; and
...
This is a relaxation of a stronger notion of  \emph{smooth approximation} known in model theory. There, the family $\Bclass$ is independent of $d$; 
...
Consider the rational numbers with the usual order. The finite substructures in this case are finite linear orders, and already for dimension $d=1$, a finite linear order of size $n$ will have $n$ orbits. 
So no family $\B$ satisfies both condition (\ref{item:oligomorphic-approximation-embedding}) and (\ref{item:oligomorphic-approximation-orbits}).
\end{non-example}
...
\begin{proof}[Proof of Theorem~\ref{thm:have-oligomorphic-approximation}]
    ...
    \begin{enumerate}
        ...
        These are all homogeneous: we can extend a bijection to a permutation.
        ...
        These are all homogeneous, since we can extend linear bijections by extending a linearly independent set to a basis. 
        ...
        Essentially, the classification of finite homogeneous graphs \cite[p.~100]{Gardiner1976} means (1) cannot hold.} 
...
$
 ...
 \lineqfun { \Lin \B^d} {\Lin \B^d},\tag{$*$}
    ...
    \begin{lemma}\label{lem:bound-on-dim-for-finite-set} 
        For every  $\B \in \Bclass$ the dimension of the vector space  in~\eqref{eq:endo-maps-b-d} is at most the number of orbits in $\B^{2d}$.
    \end{lemma}
    \begin{proof}

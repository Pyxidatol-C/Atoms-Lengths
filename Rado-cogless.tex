...
\paragraph*{Weak smooth approximation.} 
\begin{definition}
    [Weak smooth approximation]
    \label{def:weak-smooth-approximation} We say that a structure  $\A$ has \emph{weak smooth approximation} if for every $d \in \set{1,2,\ldots}$ there exists a family $\Bclass$ of finite substructures of $\A$ such that: 
    \begin{enumerate}
        \item every finite substructure of $\A$ embeds into some $\B \in \Bclass$; and
        \item there is a common finite upper bound on the number of orbits in $\B^d$ for $\B \in \Bclass$.
    \end{enumerate}
\end{definition}
...
\begin{example}[Equality atoms]\label{ex:equality-atoms-weak-smooth-approximation}
The equality atoms are oligomorphically approximable. In this case, we can choose $\Bclass$ so that is indpendent of $d$, and this is simply all finite structures with equality only. For sufficiently large $\B \in \Bclass$, the number of orbits in $\B^d$ is the same as the number of orbits in $\A^d$. 
\end{example}

\begin{example}[Bit-vector atoms]\label{ex:bit-vector-atoms-weak-smooth-approximation}
Consider the bit-vector atoms. This is the vector space over the two-element field of countably infinite dimension, which is viewed as a structure by having a binary function for addition of vectors. As in the previous example, we can choose $\Bclass$ independently of $d$, namely the family of vector spaces of finite dimension. If we changed the field to any finite field, the same argument would apply.
\end{example}
...
\begin{example}[Rado graph]\label{ex:rado-graph-weak-smooth-approximation}
Consider the Rado graph. For every $n \in \set{1,2,\ldots}$ define a finite graph as follows. The set of vertices is $2^{2n}$, which is seen as a vector space over the two-element field whose basis is 
\begin{align*}
\set{e_1,\ldots,e_n, f_1,\ldots,f_n}.
\end{align*}
Since the field has two elements, we can view vertices as subsets of the above basis. In this graph, there is an edge $vw$ if and only if the sets 
\begin{align*}
\setbuild{ i \in \set{1,\ldots,n}}{$e_i \in v$ and $f_i \in w$}\\
\setbuild{ i \in \set{1,\ldots,n}}{$f_i \in v$ and $e_i \in w$}
\end{align*}
have different sizes modulo two. This graph might seem a bit articial, it is in fact a well-known construction, which is called the \emph{symplecitic graph} over the two-element field; see e.g.~\cite{BCP97}. These graphs are placed in more context in the appendix, where we also prove that they satisfy the necessary properties from Definition~\ref{def:weak-smooth-approximation}. actually 
\end{example}

The main result of this section is the following theorem.

\begin{theorem}\label{thm:weak-smooth-approximation-finite-length}
    If an oligomorphic structure $\A$ has weak smooth approximation, then it has the finite length property over any field of characteristic $0$.
\end{theorem}

Before proving the theorem, let us apply it to prove some results -- old and new -- on the finite length property.
...
\end{corollary}
\begin{proof}
    In Examples~\ref{ex:equality-atoms-weak-smooth-approximation}, \ref{ex:bit-vector-atoms-weak-smooth-approximation}, and \ref{ex:rado-graph-weak-smooth-approximation} we have established that these structures satisfy the assumptions of  Theorem~\ref{thm:weak-smooth-approximation-finite-length}.
\end{proof}
...
\begin{proof}
    ...
    and we assumed the linear independence of $e_{n+i+1}, \dots, e_{n+k}$.
...
\begin{proposition}[Witt Extension] \label{prop:symplectic-witt-extension}
    ...
    can be extended to an automorphism of $\W_n$ (i.e., an isometric linear bijection)
...
\begin{remark}
    ...
    in $\G^k$ there are $\sum_{d=0}^k \left\{\begin{matrix}k \\ d \end{matrix}\right\} \cdot 2^{\binom{d}{2}}$ orbits 
...
\begin{proof}
    ...
    and that it may be extended to an automorphism $\pi$ of $\W_\infty$ by Propsoition~\ref{prop:symplectic-witt-extension}.

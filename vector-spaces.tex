...
\begin{myexample}\label{ex:length-one-dim-one}
    Let $\A$ be the equality atoms and let $\field$ be an arbitrary field. As explained in~\cite[Ex.~4.2]{BFKM24}, the corresponding vector space $\Lin_\field \A$
    has only three equivariant subspaces: the zero subspace, the full space, and the subspace that consists of those vectors whose coefficients add up to zero. 
\end{myexample}

...
As mentioned in the introduction, the finite length property can be strictly stronger than the ascending chain property, as witnessed by vector space atoms from Example~\ref{ex:vector-space-atoms}.  (Note for vector space atoms two fields are involved: the finite field used to define $\A$, and the field $\field$ used to define vector spaces over the atoms. In the counterexample, both fields are the two-element field.)
...
The main contribution of this paper is to  establish the finite length property for more structures, including the Rado graph (Example~\ref{ex:rado-graph}) and the vector space atoms (Example~\ref{ex:vector-space-atoms}). For the former, we will not make any assumptions on the field, but for the latter we will need to assume that the field $\field$ has characteristic zero. 

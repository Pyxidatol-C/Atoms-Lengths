...
\begin{myexample}\label{ex:length-one-dim-one}
    ...
    the zero subspace, the full space, and the subspace that consists of those vectors whose coefficients add up to zero 
    (e.g.~$a + b - 2 c$ with $a, b, c \in \A$ distinct). 
\end{myexample}
 ...
 In particular, these spaces are not closed under taking equivariant subspaces or quotients under such spaces. 
 \color{magenta}example: equality atoms, $(a, b) / \langle (a, b) + (b, c) - (a, c) \rangle \mapsto a - b$ gives an isomorphism $\Lin \A^{(2)} / \langle \cdots \rangle \cong \langle a - b \rangle$\color{black}. 
 ...
 \textcolor{magenta}{Cite Arka's thesis}
 For this reason, we use a more general notion of vector space, which is presented below, in a style that emphasises the group action, as was done in Definition~\ref{def:orbit-finite-set-alternative}.
...
\begin{definition}[Orbit-finitely spanned vector space, abstractly]\label{def:orbit-finitely-spanned-abstract}
    ...
    \begin{enumerate}
        ...
        vector addition and scalar multiplication are equivariant (where the action on the field is trivial);%
...
The above definition is easily seen to be closed under taking quotients by equivariant subspaces. 
...
We say that an atom structure $\A$ has the \emph{ascending chain property} over a field $\field$ if any of the equivalent conditions in Theorem~\ref{thm:ACP} are satisfied. 

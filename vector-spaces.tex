...
As mentioned in the introduction, the finite length property can be strictly stronger than the ascending chain property, which arises using the vector space atoms from Example~\ref{ex:vector-space-atoms}.  (Note that when we talk about the vector space atoms, there  two fields involved, namely the finite field used to define $\A$, and the field $\field$ used to define the vector space with atoms. In the counterexample, both fields are the two-element field.)
...
%including the Rado graph (Example~\ref{ex:rado-graph}) and the vector space atoms (Example~\ref{ex:vector-space-atoms}). For the former, we will not make any assumptions on the field, but for the latter we will need to assume that the field $\field$ has characteristic zero. 

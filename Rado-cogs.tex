...
Let $\mathcal{O}_1 \subseteq \A^{I_1}, \dots, \mathcal{O}_m \subseteq \A^{I_m}$ all be $S$-rigid orbits.
Then $\len(\Lin_\FF(\mathcal{O}_1 \uplus \dots \uplus \mathcal{O}_m)) = 2^{|I_1|} + \dots + 2^{|I_m|}$;
in fact, we know and can characterise all the $\Aut(\A)_{(S)}$-equivariant subspaces of $\Lin_\FF(\mathcal{O}_1 \uplus \dots \uplus \mathcal{O}_m)$.

...
Nontheless they do have the finite length property over any field \cite[Theorem~4.8]{BFKM24}, not just those of characteristic zero.
\textcolor{red}{
We give an adaptation of the proof there in this section to richer ordered structures.
In particular, we will deduce the finite length property for the Rado graph and the, even with finitely many constants fixed.}

\textcolor{red}{
In this section, we state the setting and results.
The next section is devoted to the proof.
The section after that, we show an application.
...
We devote the next section to proving Theorem~\ref{thm:ordered-free-amalg-has-finite-length}.
For now, we state a consequence and give some examples.
...
We will proceed in several steps, with the general idea similar to \cite[]{BFKM24}, but with significant new complications arising from the presence of nontrivial relations in $\A_0$.
%The zeroth step is to assume that in $\A$, every binary relation $R \in \sigma_0$ is interpreted irreflexively. 
%This is a harmless assumption: we can always split $R$ into a unary predicate $R_=$ and a binary, irreflexive relation $R_\neq$ without changing the automorphism group.
...
For the special case of the ordered atoms (Ex.~ \ref{ex:order-atoms} \&~\ref{ex:geneircally-ordered-equality}) the following construction was studied  in~\cite{BFKM24}, and it was introduced earlier in~\cite{Gray97} under the name of ``polytabloids''.
...
In light of Propositions~\ref{prop:reduction-to-one-orbit},~\ref{prop:reduction-to-kernel} and~\ref{prop:cogs-are-balanced}, for the finite length property it is enough to prove that $\Ker_\FF \cal O \subseteq \Cog_\FF \cal O$. In words, we need to show that every balanced vector in $\Lin_\FF{\cal O}$ is a linear combination of $\cal O$-cogs. Before we show a proof of this (stated as Theorem~\ref{thm:cog-span-generally}), let us illustrate its key ideas on an example.
...
Note that if a $\pi \in \Aut(\A)_{(S)}$ fixes every atom in $v^{j:a}$ --- in particular, $a$ ---  then
\[

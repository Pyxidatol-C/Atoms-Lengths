...
Note the image $\mathcal{O}|^J$ of $\mathcal{O}$ under this projection agrees with $\Aut(\A)_{(S)} \cdot o |^J$ and is still ordered.
...
Note that, for a fixed $S$-ordered orbit $\cal O$, all $\mathcal{O}$-duos are in the same $\Aut(\A)_{(S)}$-equivariant orbit.
...
\begin{proposition}\label{prop:cogs-are-balanced}
    $\Cog_\FF \mathcal{O}$ is contained in $\Ker_\FF \mathcal{O}$.
\end{proposition}
\begin{proof}

...
\begin{definition}
   ...
   % and take any $o_\bullet \in \mathcal{O}$.
    ...
    \begin{enumerate}
        \item 
        \emph{unambiguous} if for any $a,b\in V$, if $i\neq j\in I$ then $a_i\neq b_j$; %$\sqrt{-} : b^{(k)}_i \mapsto o_i$ for $k \in K$ is a function --- i.e., if $i = i'$ whenever $b^{(k)}_i = b^{(k')}_{i'}$;
...
%    But this is impossible: $\vsup{v}$ and $V_{j-1}$ are both unambigous, so $i=i''$ and $i''=i'$.
    ...
    Since $V_{j-1}$ is unobstructed, $b$ (or, symmetrically, $c$) must be one of the $x^{(a_j, k)}_\bullet ; a_j$ or $y^{(a_j, k)}_\bullet ; a_j$. 
   % Finally, notice that by the claim for $j - 1$, an obstruction $b^{(k)}_i$ must again come from $b^{(k)}_\bullet$ being some $x^{(a_j, k)}_\bullet ; a_j$ or $y^{(a_j, k)}_\bullet ; a_j$.
    ...
    \[
        V_{j-1} \cup \{ (\tau \cdot x^{(a_{j}, k)}_\bullet ; a_{j}), (\tau \cdot y^{(a_{j}, k)}_\bullet ; a_{j}) \mid k \in K_{j}\}, 
    \]
    remains unambiguous.
...
Having chosen the fresh atoms carefully, let $X$ consist of all the atoms appearing in the unobstructed family $V_m$.

...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item 
        \emph{free enough} if it is well-indexed and for any $b^{(k)}_i \neq b^{(k')}_{i'}$, either $\A \models R(b^{(k)}_i, b^{(k')}_{i'}) \leftrightarrow R(o_i, o_{i'})$ for every binary relation $R \in \mathcal{L}_0$ and some/any $o_\bullet \in \mathcal{O}$,
...
Now suppose that some $i_0 \in I$ exists --- in fact, let $i_0$ be minimal.
Consider a vector $v \in \Ker_\EE \mathcal{O}$ with $\mathcal{O}[v]$ being free enough.
We can write
\[
    v = \sum_{a \in A} v^{i_0 : a},
\]
decomposing $v$ into finitely many subvectors.
Each projection $v^{i_0 : a}|^{-i_0}$ belongs to $\Ker_\EE \mathcal{O}^{i_0:a}|^{-i_0}$ by Lemma~\ref{lem:balanced-projected-subvector}.
Moreover:
\begin{claim}
    $\mathcal{O}^{i_0:a}|^{-i_0}[v^{i_0:a}|^{-i_0}]$ is free enough.
\end{claim}

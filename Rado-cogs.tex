...
\begin{proof}
    ...
    Hence the \textcolor{red}{$S$-length} of $\Lin_\FF \A^d$ equals \[
...
Moreover, thanks to irreflexivity \textcolor{red}{TODO: explain somewhere that we assume this!}, each $a_i$ is \textcolor{red}{unrelated} to its counterpart $b_i$.
...
As a result, $\Cog_\FF \mathcal{O}$ is an $\Aut(\A)_{(S)}$-equivariant subspace of $\Lin_\FF \mathcal{O}$ and it is generated by 
any single cog.
...
Let us rectify this by showing that they can be found almost everywhere.
...
\begin{lemma}\label{lem:cog-fresh-single}
 ...
 %       \item let $Y \subseteq \A$ be any finite set disjoint from $X \cup \{z\}$;
    \end{itemize}
    Denote $z'=\tau(z)$, where $\tau \in \Aut(\A)_{(X)}$ is afforded by Lemma~\ref{lem:free-fresh} (putting $Y=\emptyset$ \textcolor{red}{--- later we use it with bigger $Y$'s}). Then $az \parallel bz'$ is an $\mathcal{O}'$-duo.
\end{lemma}
\begin{proof}
...
The result below \textcolor{red}{strengthens} Proposition~\ref{prop:cogs-are-balanced}, apart from the fact that the latter also implies that $\Ker_\FF{\cal O}$ is nontrivial.
...
%    Let $V \subseteq \Cog_\FF \mathcal{O}$ be a non-zero $\Aut(\A)_{(S)}$-equivariant subspace.
 ...
 By Theorem~\ref{thm:cogs-arise-everywhere}, $\Cog_\FF \mathcal{O}$ \textcolor{red}{is nontrivial and} has no nontrivial $\Aut(\A)_{(S)}$-equivariant subspaces.
\end{proof}

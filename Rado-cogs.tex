...
$\Lin_\EE \cal O$ and $\Ker_\EE \cal O$ are still vector spaces over $\FF$.
\end{remark}
...
\begin{theorem}\label{thm:coeff-approximation}
    ...
    \end{align*}
    where $\cal O_i$ is an orbit of $\A^e$ with $e < d$, and $n_i \in \set{1,2,\ldots}$, with the following property.  
    Every equivariant subspace $W \subseteq \Lin_\FF \A^d$ is equal to
...
\textcolor{red}{These ideas are in some capacity present in \cite[Cor.~3.17]{Gray97}, \cite{HofmanRozycki_2022}, \cite{ghosh2023orbit}, and in Arka's notes. Cor: this tightens the bound in \cite{BFKM24}.}

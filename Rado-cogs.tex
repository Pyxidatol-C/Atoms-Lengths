...
(If $\A$ is the ordered atoms, we can simply take $z$ to be the largest atom present in $v$: cf.~the proof of \cite[\text{Cl.~4.7}]{BFKM24}.)
...
\begin{theorem}\label{thm:coeff-approximation}
    ...
    \]
    where $\cal O_i$ is an ordered orbit of $\A^{d_i}$ with $d > d_i$, 
...
In the orbit-finite setting, we assume that the rows and columns are indexed by $\A^n$, that every column has finitely many non-zero entries, and that $j \mapsto \mathbf{A}_{-, j}$ is equivariant.
That is, we ask whether $\mathbf{b} \in \Lin_\FF \A^d$ is in the span of $\mathbf{A}_{-, j}$. 
By Theorem~\ref{thm:coeff-approximation}, it suffices to check whether $\mathbf{b} {\restriction_i} (\cal O_i) \subseteq \mathbf{A}_{-, j_1} {\restriction_i} (\cal O_i) + \cdots + \mathbf{A}_{-, j_n} {\restriction_i} (\cal O_i)$, in the finite dimensional space $\FF^{n_i}$, having chosen orbit representatives $j_1, \dots, j_n \in \A^d$.

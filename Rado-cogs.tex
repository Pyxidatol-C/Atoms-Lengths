...
\subsection{Cogs}
\textcolor{red}{BK: Hopefully this is equivalent to the original def.?}
\begin{definition}
    Let $\mathcal{O} = \Aut(\A)_{(S)} \cdot o_\bullet \subseteq \A^I$ be an $S$-ordered orbit. 
 ...
 %   $2|I|$ atoms in $\A\setminus S$ such that: with the following $\mathcal{L}$-structure on $\{a_i, b_i \mid i \in I\} \cup S$:
    \begin{enumerate}
       ...
       % $a_{i_1} < b_{i_1} < a_{i_2} < b_{i_2} < \dots < a_{i_d} < b_{i_d}$ where $I$ consists of the indices $i_1 < i_2 < \dots < i_d$;
        ...
        \item 
        for any binary relation $R$ in $\mathcal{L}_0$ (except for $=$):
        \[ \A \models R(a_i,b_j) \iff \A \models R(b_i,a_j) \iff \A \models R(o_i,o_j).\] 
     \end{enumerate}
\end{definition}
...
Third, given any $J \subseteq I$, the combined tuple $a|^J_\bullet ; b|^{I \setminus J}_\bullet$ lies in $\mathcal{O}$ by homogeneity:
observe
\begin{align*}
...
Finally, by homogeneity still, all $\mathcal{O}$-duos are in the same $\Aut(\A)_{(S)}$-equivariant orbit.
...
\end{lemma}
\textcolor{red}{BK todo: Simplify this proof accordingly.}
\begin{proof}
    First, notice that $b_\bullet ; \tau(z)\in {\cal O}'$ and that we have the required order relations with $z$ and $\tau(z)$.

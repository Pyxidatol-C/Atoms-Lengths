...
In particular, we will deduce the finite length property for the Rado atoms and its variants, even with finitely many constants fixed.
...
\begin{theorem}\label{thm:ordered-free-amalg-has-finite-length}
    Consider:
    \begin{itemize}
        \item[$\sigma_0$] a finite, at-most-binary relational vocabulary;
        \item[$\Cclass_0$] a free amalgamation class of finite, irreflexive $\sigma_0$-structures;
        \item[$\Cclass$] all total orderings of all structures in $\Cclass_0$, over the vocabulary $\sigma_0 \uplus \{<\}$;
        \item[$\A$] the Fraïssé limit of $\Cclass$ --- a homogeneous and oligomorphic $(\sigma_0 \uplus \{<\})$-structure.
    \end{itemize}
...
\begin{myexample}\label{ex:generically-ordered-equality}
...
\begin{myexample}\label{ex:generically-ordered-graphs}
...
\begin{claim}\label{claim:reduction-to-kernel}
    ...
    \begin{enumerate}
        \item $\Lin_\FF \cal O$ has finite length with respect to $\Aut(\A/S)$ for every $S$-ordered orbit $\cal O$;
        \item $\Ker_\FF \cal O$ has finite length with respect to $\Aut(\A/S)$ for every $S$-ordered orbit $\cal O$.
    \end{enumerate}
\end{claim}
\begin{proof}
...
For the special case of the ordered atoms (Examples~\ref{ex:order-atoms} \&~\ref{ex:generically-ordered-equality}) the following construction was studied in~\cite[p.~11]{BFKM24}, and it already appeared earlier in~\cite[p.~125]{Gray97} under the name of ``polytabloids''.

...
\begin{myexample}
    ...
    For $d = 1$, the maps are
    \begin{align*}
        v {\restriction_1} (a) &= v(a) \\
        v {\restriction_2} () &= \sum_{-} v(-)
    \end{align*}
    This shows that $\Lin \A$ has the same structure as the space over the equality atoms in Example~\ref{ex:length-one-dim-one}.
\end{myexample}


The ideas of using these local coefficients to enumerate equivariant subspaces can be found in \cite[Cor.~3.17]{Gray97}, \cite{HofmanRozycki_2022}, and \cite{ghosh2023orbit}.

\paragraph*{Systems of linear equations.}
Solving a linear equation $\mathbf{A x} = \mathbf{b}$ amounts to checking whether $\mathbf{b}$ is spanned by the columns of $\mathbf{A}$.
As explained in \cite[Sec.~9]{GhoshLasota_24}
In the orbit-finite setting, we assume that every column has finitely many non-zero entries, and that $j \mapsto \mathbf{A}_{-, j}$ is equivariant.
Theorem~\ref{thm:coeff-approximation} allows us to check membership in an orbit-finitely spanned subspace.
The column-finite assumption is without loss of generality over the ordered atoms (which has the function space property),
and we can relax equivariance to finite supports because the structure interprets constants.

\paragraph*{Precise lengths.}
The characterisation gives us an upper bound on $\Lin_\FF \cal O$, which meets with the lower bound given by \cite[Cor.~4.12]{BFKM24}.

...
\begin{definition}
    ...
    \begin{enumerate}
        \item 
        \emph{rooted} if $\sqrt{-} : b^{(k)}_i \mapsto o_i$ for $k \in K$ is a function --- i.e., if $i = i'$ whenever $b^{(k)}_i = b^{(k')}_{i'}$;

        \item 
        \emph{free enough} if it is well-indexed and for any $b^{(k)}_i, b^{(k')}_{i'}$, 
        ...
        \end{align*}
        for every binary relation $R \in \mathcal{L}_0$.%
        \footnote{Of course, whether the family is rooted or free enough does not depend on the choice of the specific representative $o_\bullet \in \mathcal{O}$!}
    \end{enumerate}
\end{definition}

We can now state the precise result.
\begin{theorem}\label{thm:cog-span-special-case}
    Let $v \in \Ker_\EE \mathcal{O}$ be such that $\mathcal{O}[v]$ is free enough.
...
The base case $I = \emptyset$ is immediate as before:
as $\mathcal{O}$ is the singleton $\{ () \}$,
any vector $v \in \Ker_\EE \mathcal{O} = \Lin_\EE \mathcal{O} = \Cog_\EE \mathcal{O}$ is a cog already, where $\mathcal{O}[v]$ is vacuously free enough;
so the theorem says nothing more than $v = v$.

Now suppose that some $i^* \in I$ exists --- in fact, let $i^*$ be maximal.
Consider a vector $v \in \Ker_\EE \mathcal{O}$ with $\mathcal{O}[v]$ being free enough.
...
and we may straightforwardly check that \textit{a fortiori} $\mathcal{O}^{i^* : a_j}|^{-i^*}[v^{i^* : a_j}|^{-i^*}]$ is rooted and free enough.
...
$\mathcal{O}^{i^* : a_j}|^{-i^*}[v^{i^* : a_j}|^{-i^*}] \cup \{x^{(a_j, k)}_\bullet, y^{(a_j, k)}_\bullet \mid k \in K_j\}$ is free enough.
...
\]
is easily seen to be free enough.
We can ask for more:
...
\begin{claim}\label{claim:half-cog-freshened-up}
    ...
    \]
    is free enough for $j = 1, 2, \dots, m$.
\end{claim}
\begin{proof}
    Let $A_j$ denote the atoms that appear in $\mathcal{O}[v^{i^* : a_j}]$.
    Given any automorphism $\pi \in \Aut(\A)$ that fixes $S$ as well as $A_j$, 
    using the tuples $\pi \cdot x^{(a_j, k)}_\bullet$ and $\pi \cdot y^{(a_j, k)}_\bullet$ gives us another decomposition of $v^{i^* : a_j}|^{-i^*}$ that is free enough.
    ...
    Suppose that the family in the claim for $j$ is not even rooted.
    ...
    This is impossible: we know two families that are free enough which imply $i = i''$ and $i'' = i'$.
    
    So the problematic atom $b^{(k)}_i$ does not belong to $A_j$.
    ...
    \end{align*}
    the problematic atoms are precisely the ones in the old family minus $b^{(k)}_i$ --- they are all fixed by $\tau$.
    We thus continue this way until all problematic atoms are freshened.

    Hence assume that the family in the claim for $j$ is rooted.
    Suppose $b^{(k)}_i$ and some $b^{(k')}_{i'}$ are the reason why the family fails to be free enough;
    call the atom $b^{(k)}_i$ an obstruction.
    To have $b^{(k)}_i \in A_j$ is impossible:
    otherwise $b^{(k)}_i = b^{(k'')}_{i''}$ for some $b^{(k'')}_\bullet \in \mathcal{O}[v^{i^* : a_j}] \subseteq \mathcal{O}[v]$;
    then $i = i''$ by the rooted assumption, yet $b^{(k'')}_{i''}$ --- hence $b^{(k)}_i$ --- cannot be an obstruction because we have two subfamilies that are free enough.
...
Having chosen the fresh atoms carefully, let $X$ consist of all the atoms appearing in $\mathcal{O}[v] \cup \{ (x^{(a_{j'}, k)}_\bullet ; a_{j'}), (y^{(a_{j'}, k)_\bullet; a_{j'}}) \mid k \in K_{j'}, 1 \leq j' \leq m\}$.
The trick now is to add a new element $b^*$ to the finite $\mathcal{L}$-structure $S \cup Z \subseteq \A$. 
We define the new relations by
\[\left\{
\begin{aligned}
    R_1(b^*) &\iff R_1(\sqrt{b^*}) \\
    R_2(b^*, s) &\iff R_2(\sqrt{b^*}, \sqrt s) \\
    R_2(s, b^*) &\iff R_2(\sqrt s, \sqrt{b^*}) \\
    R_2(b^*, b^{(k)}_i) &\iff R_2(\sqrt{b^*}, \sqrt{b^{(k)}_i}) \\
    R_2(b^{(k)}_i, b^*) &\iff R_2(\sqrt{b^{(k)}_i}, \sqrt{b^*}) \\
    b^* > s &\iff \sqrt{b^*} > \sqrt s \\
    b^* > b^{(k)}_i &
\end{aligned}
\right.\]
...
\]
by letting $\sqrt{s} = s$ and $\sqrt{b^*} = o_{i^*}$;
...
\begin{proof}
    ...
    If not, by Lemma~\ref{lem:free-forb} there is a forbidden $\mathcal{L}_0$-structure $F$ which embeds into $S \cup X \cup \{b^*\}$ via $\phi$.
    We will show that $\sqrt{-} \circ \phi$ then embeds $F$ into $\A$, which is clearly impossible.
...
\begin{proof}
    ...
    yielding $0 = \sum_{j=1}^m v^{i^* : a_j}|^{-i^*} ; b^*$ as desired.
\end{proof}

\begin{claim}
    The family $\mathcal{O}[v] \cup \{ (x^{(a_j, k)}_\bullet ; a_j),  (y^{(a_j, k)}_\bullet ; b^*) \mid k \in K_j, 1 \leq j \leq m \} $ is free enough.
\end{claim}
\begin{proof}
    \color{red} TODO 3
\end{proof}

\subsection{Reducing to the special case}

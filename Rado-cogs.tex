...
Throughout this and the next two sections, we will work with the following setting.

\paragraph*{Graph vocabulary}
Consider a finite relational vocabulary $\sigma_0$ consisting of unary and binary symbols.
...
There is a more useful characterisation \cite[Lemma~4.5]{SS20} of the amalgamation class $\Cclass_0$ being free.
Since $\Cclass_0$ is closed under substructures and isomorphisms, 
we know that $\Cclass_0$ consists of all the finite $\sigma_0$-structures which do not embed any structure from $\Fclass$, 
where $\Fclass$ consists of all the minimal finite $\sigma_0$-structures that do not belong to $\Cclass_0$;
we write \[
    \Cclass_0 = \Forb(\Fclass).
\]
That $\Cclass_0$ is close under free amalgamation means that in each $F \in \Fclass$, 
...
Conversely, given a family $\Fclass$ of finite $\sigma_0$-structures in each of which any two elements are related,
the class $\Forb(\Fclass)$ of finite $\sigma_0$-structures form a \emph{free} amalgamation class.
...
\begin{example}\label{ex:free-amalg-graphs}
    Let $\sigma_0$ consist of a single binary symbol $\sim$. 
    Consider $\sigma_0$-structures ${\circlearrowright} = \{x\}$ and ${\rightarrow} = \{y, z\}$, where the relation $\sim$ is interpreted as $\{(x, x)\}$ and $\{(y, z)\}$.
...
Now, let $\sigma$ consist of $\sigma_0$ together with a new binary symbol $<$.
...
Moreover, it follows from a back-and-forth argument \cite[Lemma~7.1.4]{Hodges93} that the $\sigma_0$-structure $\A$ is also homogeneous and, therefore, that it is isomorphic to $\A_0$.
...
\textcolor{red}{Before anything, let us get a technical assumption out of the way:
we may assume that in $\Cclass_0$ and hence in $\A$, every binary relation $R \in \sigma_0$ is interpreted irreflexively.}
...
$\cal O = \Aut(\A)_{(S)} \cdot a$ only contains $S$-ordered tuples, and we will call the orbit $S$-ordered as well.
...
So fix an ordered orbit $\cal O = \Aut(\A)_{(S)} \cdot o \subseteq \A^I$.
...
The image $\cal O|^J$ of $\cal O$ under this projection agrees with $\Aut(\A)_{(S)} \cdot o |^J$ and is still ordered.
...
\end{definition}
Note that, for a fixed $S$-ordered orbit $\cal O$, all $\cal O$-duos (hence all $\cal O$-cogs) are in the same $\Aut(\A)_{(S)}$-equivariant orbit.
As a result, $\Cog_\FF \cal O$ is an $\Aut(\A)_{(S)}$-equivariant subspace of $\Lin_\FF \cal O$ and it is generated by any single cog.
...
\begin{lemma}\label{lem:cog-fresh-single}
 ...
 %       \item let $Y \subseteq \A$ be any finite set disjoint from $X \cup \{z\}$;
    \end{itemize}
    Denote $z'=\tau(z)$, where $\tau \in \Aut(\A)_{(X)}$ is afforded by Lemma~\ref{lem:free-fresh} (with an arbitrary $Y$). Then $az \parallel bz'$ is an $\cal O'$-duo.
\end{lemma}
\begin{proof}
...
\begin{theorem} \label{thm:cogs-arise-everywhere}
    Any non-trivial $\Aut(\A)_{(S)}$-equivariant subspace $V\subseteq \Lin_\FF \cal O$ contains $\Cog_\FF \cal O$.
\end{theorem}
\begin{proof}
    ...
    By Proposition~\ref{prop:cog-fresh-full}, we can find $b \in \cal O'$ such that $a \parallel b$ is an $\cal O'$-duo and \emph{a fortiori} an $\cal O$-duo.
...
\end{itemize}
We resolve such {\em conflicts} by considering auxiliary atoms $g'>g$ and $b'>b$, with just enough edges to make $gh\parallel g'i$ and $bf\parallel b'i$ valid ${\cal O}$-duos. Specifically, we postulate edges $g' \sim h$, $g'\sim i$, $b' \sim f$ and $b' \sim i$ and no more. Such atoms exist by Lemma~\ref{lem:free-fresh}. We then define:
\[
...
Inspired by the previous example, we shall now prove that $\Ker_\FF \cal O \subseteq \Cog_\FF \cal O$ for any $S$-ordered orbit $\cal O\subseteq \A^I$.
...
\[
    \cal O^{i:a} = \{b \in \cal O \mid b_i = a\};
\]
this is an $\Aut(\A)_{(S a)}$-orbit, and its projection $\cal O^{i:a} |^{-i}$ is $Sa$-ordered.
...
For a finite subset $T \subseteq \cal O$, a {\em location} in $T$ is a pair $(i,a)\in I\times\A$ such that $a=c_i$ for some $c\in T$. Note that for any fixed $i,j\in I$, for all $c\in\cal O$ the atoms $c_i$ and $c_j$ are related in the same way in $\A$. We say that two locations $(i,a)$ and $(j,b)$ in $T$ are in:
\begin{itemize}
...
%
%Let $v \in \Ker_\EE \cal O$ and let $v^{i:a_i}$ be a subvector.
...
\begin{lemma}\label{lem:?}
    Let $K, V_0, V$ be finite subsets of $\cal O$ such that $V_0 \subseteq V$ and both $V_0\cup K$ and $V$ are free from equational conflicts. Then there exists a $\pi\in\Aut(\A)$ that fixes all atoms in $S$ and in $\rt{V_0}$, and such that $V \cup \pi(K)$ is free from equational conflicts.
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lem:!}
    Let $K, V_0, V$ be finite subsets of $\cal O$ such that $V_0 \subseteq V$ and both $V_0\cup K$ and $V$ are free from relational conflicts. Then there exists a $\pi\in\Aut(\A)$ that fixes all atoms in $S$ and in $\rt{V_0}$, and such that $V \cup \pi(K)$ is free from relational conflicts.
\end{lemma}
\begin{proof}
...
\begin{enumerate}
    ...
    \end{enumerate}
    These are well-defined because $a^\pm a, b^\pm b \in \cal O$ and $i = j$ whenever $a^\pm_i = b^\pm_j$.
\end{enumerate}
...
Then, for any $a^\pm \in A^\pm$, it is \textcolor{red}{easy (using \ref{lem:cog-fresh-single})} to check that $a^+ a \parallel a^- a'$ is an $\cal O$-duo. Moreover, 
...
Then, for any $a^\pm \in A^\pm$, it is easy to check that $a^+ a \parallel a^- a'$ is an $\cal O$-duo. Moreover, 
the vector
\begin{align*}
    v' = v - \sum_{a^\pm\in A^\pm} \lambda_{a^\pm} \cdot a^+ a \between a^- a'
    &= v -  v^{i:a} + v^{i:a'} 

...
\begin{itemize}
    \item 
    $\mathcal{L}_0$ is a \textcolor{red}{(should we just assume finite?)} relational language \textcolor{red}{consisting of unary and binary symbols};
...
\begin{theorem}\label{thm:free-bin-oligo}
    ...
    Then $\A$ has the finite length property even with finitely many constants fixed, provided that $\A$ is oligomorphic (for instance if $\mathcal{L}_0$ is finite).
\end{theorem}
...
To start with, let us view $\A^d$ as $\A^{\{1, \dots, d\}}$ and more generally consider $\A^I$ for a finite indexing set $I \subseteq \Q$.
...
\begin{proposition}\label{prop:reduction-to-one-orbit}
    ...
    \begin{enumerate}
        ...
        \item $\A$ is oligomorphic, and $\Lin_\FF \mathcal{O}$ has fintie length for any ordered orbit $\mathcal{O}$.
    \end{enumerate}
\end{proposition}
\begin{proof}
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item 
        any relation in $\mathcal{L}_0$ except for $=$ holds for a tuple $c_\bullet$ with entries in $a_I \cup b_I \cup S$ if and only if it holds for $c_\bullet$ with each entry equal to $a_i, b_i$ replaced by $o_i$.
    \end{enumerate}
\end{definition}
...
\begin{lemma}\label{lem:cog-fresh-single}
    Suppose $a_\bullet \parallel b_\bullet$ is an $\mathcal{O}$-cog duo, where $\mathcal{O} \subseteq \A^I$ is $S$-ordered.
    Given $z \in S$, 
    \begin{itemize}

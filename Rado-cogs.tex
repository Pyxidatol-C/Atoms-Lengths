...
\begin{proposition}
    ...
    \[
        \ddot v = v - \sum_{k \in K_0} v(b^{(k)}_\bullet) \cdot x^{(k)}_\bullet \between y^{(k)}_\bullet
    \]
    satisfies $\ddot v(\mathcal{O}) \subseteq v(\mathcal{O}) \subseteq \EE$ and $\mathcal{O}(\ddot v)$ is unambiguous.
\end{proposition}
\begin{proof}
    ...
    --- in fact, we will use a variant so that $\tau \cdot a_i < a_i$. 
    It follows from the appropriate modification of Lemma~\ref{lem:cog-fresh-single} that we get $\mathcal{O}$-cog duos $x^{(k)}_\bullet ; \tau \cdot a_i \parallel y^{(k)}_\bullet ; a_i$ for $k \in K$.
    Observe that
    \begin{align*}
        v^{i:a_i} &+ \sum_{k \in K} v(b^{(k)}_\bullet ; a_i) \cdot ( x^{(k)}_\bullet ; \tau \cdot a_i \between y^{(k)}_\bullet ; a_i ) \\
        = v^{i:a_i} &- \sum_{k \in K} v(b^{(k)}_\bullet ; a_i) \cdot ( x^{(k)}_\bullet \between y^{(k)}_\bullet ) ; a_i \\
        &+ \sum_{k \in K} v(b^{(k)}_\bullet ; a_i) \cdot ( x^{(k)}_\bullet \between y^{(k)}_\bullet ) ; \tau \cdot a_i
    \end{align*}

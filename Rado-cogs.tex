...
Further, given any $J \subseteq I$, the combined tuple $a|^{I\setminus J} ; b|^J$ satisfies the same relations as $a ; b$, so it lies in $\mathcal{O}$. In particular, taking $J=\{i\}$, there is an automorphism $\pi_i$ that sends $a_i$ to $b_i$ and fixes all the other elements of $a$, $b$ and $S$.
Finally, all $\mathcal{O}$-duos are in the same $\Aut(\A)_{(S)}$-equivariant orbit.
\end{remark}
\begin{definition}
    Given $\lambda \in \EE$ and an $\mathcal{O}$-duo $a \parallel b$, the corresponding \emph{$\mathcal{O}$-cog} with coefficient $\lambda$ is the vector
    \[
        \lambda \cdot a \between b =
        \sum_{J \subseteq I} (-1)^{|J|} \lambda \cdot  a|^{I \setminus J} ; b^{J}
    \]
    in $\Lin_\EE \mathcal{O}$.
    The linear span of all $\mathcal{O}$-cogs with coefficients from $\EE$ is denoted by $\Cog_\EE \mathcal{O}$.
\end{definition}
As remarked above, given any two $\mathcal{O}$-duos there is some $\pi \in \Aut(\A)_{(S)}$ such that $\pi \cdot (a \parallel b) = a' \parallel b'$ and thus $\pi \cdot (\lambda \cdot a \between b) = \lambda \cdot a' \between b'$.
...
\begin{proof}
    ...
    Let $\mathcal{O} \subseteq \A^I$, let $a\parallel b$ be an $\mathcal{O}$-duo, and let $i \in I$.
    ...
    But this difference gets erased under $(-)|^{-i}$, so the two corresponding terms in $\lambda \cdot a \between b$ will cancel out and hence $( \lambda \cdot a \between b )|^{-i} = 0$ overall.
\end{proof}

...
\begin{theorem}\label{thm:ordered-free-amalg-has-finite-length}
    ...
    \begin{itemize}
        ...
        \item[$\Cclass$] all total orderings of all structures in $\Cclass_0$, viewed as $(\sigma_0 \uplus \{<\})$-structures;
        \item[$\A$] the Fraïssé limit of $\Cclass$, which is a homogeneous and oligomorphic\footnote{Every homogeneous structure over a finite relational vocabulary is oligomorphic. See ~\cite[1607--1608]{Macpherson11} for instance.} 
        $(\sigma_0 \uplus \{<\})$-structure.
    \end{itemize}
...
the \emph{projection kernel} is the $\Aut(\A/S)$-equivariant subspace of $\Lin_\FF \cal O$ below:
\[
...
We still have $\Lin_{\FF^n} \cal O \supseteq \Lin_\EE \cal O \supseteq \Ker_\EE \cal O = \Cog_\EE \cal O$ as equivariant spaces over $\FF$.
\end{remark}
...
we only need to look at local sums of coefficients.
...
\begin{theorem}\label{thm:coeff-approximation}
    ...
    \]
    where $\cal O_i$ is an ordered orbit of $\A^{d_i}$ with $d \geq d_i$, 
...
In the orbit-finite setting, we assume that the rows and columns are indexed by $\A^d$, that every column has finitely many non-zero entries, and that $j \mapsto \mathbf{A}_{-, j}$ is equivariant.

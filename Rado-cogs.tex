...
\begin{itemize}
    \item 
    $\mathcal{L}_0$ is a (possibly infinite) relational language \textcolor{red}{(NOT?) containing a binary symbol $=$};

    \item 
    $\mathcal{C}_0$ is a free amalgamation class of finite $\mathcal{L}_0$-structures
...
Let $\mathcal{O}_1 \subseteq \A^{I_1}, \dots, \mathcal{O}_n \subseteq \A^{I_n}$ all be $S$-rigid orbits.
...
\begin{proof}
    ...
    that $v {\restriction_i} = 0$ implies $0 = v_k|^{I_i} = v_k$. 
\end{proof}
...
It follows from the inductive hyothesis that \[
...
then $W = \widetilde W = \widetilde{W'} = W'$ by Theorem~\ref{thm:symplectic-smooth-approximation}.
...
\begin{proof}
    ...
    Hence $l \geq |P_1| + |P_2| + \dots + |P_t| = 2^{|I_1|} + \dots + 2^{|I_t|}$. 
\end{proof}

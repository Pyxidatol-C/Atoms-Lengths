...
As from now on we will only considered ordered orbits, we can afford a lightweight notation for combining tuples of atoms: if $a\in\A^d$ and $b\in\A^{d'}$ are both ordered and are disjoint, then by $ab$ we denote the unique ordered tuple in $\A^{d+d'}$ that arises as their union. For example, for any $S$-ordered $a\in\A^I$ and $J\subseteq I$, we have $a|^{I\setminus J}a|^J=a$.
...
\begin{proof}
   % \textcolor{red}{TODO: rewrite using the cog calculus.}
    ...
    The two tuples $a|^{I\setminus J} b|^{J}$ and $a|^{I \setminus (J \cup \{i\})}b|^{J \cup \{i\}}$ are present in $\lambda\cdot a\between b$ with the opposite coefficients, and they differ only on the $i$-th entry.
    Therefore they cancel out under $(-)|^{-i}$, hence $( \lambda \cdot a \between b )|^{-i} = 0$.
\end{proof}

...
Moreover, thanks to irreflexivity, each $a_i$ is unrelated to its counterpart $b_i$.
Further, given any $J \subseteq I$, the combined tuple $a|^{I\setminus J}_\bullet ; b|^J_\bullet$ satisfies the same relations as $a_\bullet ; b_\bullet$, so it lies in $\mathcal{O}$. In particular, taking $J=\{i\}$, there is an automorphism $\pi_i$ that sends $a_i$ to $b_i$ and fixes all the other elements of $a_\bullet$, $b_\bullet$ and $S$.
...
\begin{proposition}\label{prop:cog-fresh-full}
    Let $\mathcal{O} \subseteq \A^I$ be an $S$-ordered orbit.
    Then any $a_\bullet \in \mathcal{O}$ can be extended to an ${\cal O}$-duo $a_\bullet\parallel b_{\bullet}$.
\end{proposition}
\begin{proof}
...
For a vector $v \in \Lin_\EE \mathcal{O}$ where $\cal O$ is $S$-ordered, define:
\[
	\vsup{v} = \{a\in{\cal O} \mid v(a)\neq 0\}.
\]
Take any atom $a_i$ for $a\in\vsup{v}$, and let $P, R \in \mathcal{L}_0$ be unary and binary respectively.
Then whether $P(a_i)$, $R(a_i, s)$, $R(s, a_i)$ hold in $\A$ for some $s \in S$ is determined by $\mathcal{O}$ and $i$, and so it does not depend on the choice of $a$.

Now take another atom $b_j$ for some $b\in\vsup{v}$.
What can be said about $R(a_i, b_j)$? Unless $a=b$, not much.
The index $j$ may not even be unique: we may well have $a_i = b_j$ even if $i\neq j$.
We want to avoid such confusions:
\begin{definition}\label{def:unobstructed}
    Let $\mathcal{O}$ be an $S$-ordered orbit in $\A^I$, and fix some representative $o\in{\cal O}$.
   % and take any $o_\bullet \in \mathcal{O}$.
    Call a finite family $V\subseteq{\cal O}$ 
    %$\{ b^{(k)}_\bullet \in \mathcal{O} \mid k \in K\}$ is
    \begin{enumerate}
        \item 
        \emph{unambiguous} if the assignment $\rt{-}:a_i\mapsto o_i$ is a well-defined function on the atoms present in $V$, i.e., if for any $a,b\in V$, if $a_i=b_j$ then $i=j$; %$\sqrt{-} : b^{(k)}_i \mapsto o_i$ for $k \in K$ is a function --- i.e., if $i = i'$ whenever $b^{(k)}_i = b^{(k')}_{i'}$;

        \item 
        \emph{unobstructed} if it is unambiguous and for any $a,b\in V$ and $i,j\in I$, either $a_i$ and $b_j$ are unrelated or 
	\[
           R(a_i,b_j) \iff R(o_i,o_j) \\
         \]
    ...
    \end{enumerate}
    (Obviously these properties do not depend on the choice of $o\in{\cal O}$.) Furthermore,
    we call a vector  $v \in \Lin_\EE \mathcal{O}$ unambiguous or unobstructed if the set $\vsup{v}$ is so.
\end{definition}

For example, it is easy to see that cogs are unobstructed.

%We can now state the precise result.
\begin{theorem}\label{thm:cog-span-special-case}
    Let $v \in \Ker_\EE \mathcal{O}$ be unobstructed.
    Then we have
    \[
        v = \sum_{k \in K} v(b^{(k)}) \cdot x^{(k)} \between y^{(k)}
    \]
    for some $b^{(k)},x^{(k)},y^{(k)}\in{\cal O}$, where moreover
    \[
        \vsup{v} \cup \{ x^{(k)}, y^{(k)} \mid k \in K \}
    \]
    is unobstructed.
\end{theorem}

Note that the last part of the theorem does not trivially follow from the first: $\vsup{x^{(k)} \between y^{(k)}}$ may contain some tuples which are absent from $\vsup{v}$ due to cancellations with other cogs.
...
any vector $v \in \Ker_\EE \mathcal{O} = \Lin_\EE \mathcal{O} = \Cog_\EE \mathcal{O}$ is a cog already;
so the theorem says nothing more than $v = v$.

Now suppose that some $i^* \in I$ exists --- in fact, let $i^*$ be maximal.
Consider an unobstructed vector $v \in \Ker_\EE \mathcal{O}$.
...
and we may straightforwardly check that it is unambiguous and unobstructed.
It follows from the inductive hypothesis of Theorem~\ref{thm:cog-span-special-case} that 
\[
    v^{i^* : a_j}|^{-i^*} 
    = \sum_{k \in K_j} v^{i^* : a_j}|^{-i^*}(b^{(a_j, k)}) \cdot x^{(a_j, k)} \between y^{(a_j, k)}
\]
for some tuples $b^{(a_j, k)},x^{(a_j, k)},y^{(a_j, k)}\in\mathcal{O}^{i^* : a_j}|^{-i^*}$ such that
$\vsup{v^{i^* : a_j}|^{-i^*}} \cup \{x^{(a_j, k)}, y^{(a_j, k)} \mid k \in K_j\}$ is unobstructed.
We can return to $\mathcal{O}$ by adding $a_j$ back as the $i^*$th term to every tuple:
we get
\[
    v^{i^* : a_j} 
    = \sum_{k \in K_j} \Bigl( v(b^{(a_j, k)} ; a_j) \cdot x^{(a_j, k)} \between y^{(a_j, k)} \Bigr) ; a_j,
\]
where the family
\[
    U_j = \vsup{v^{i^* : a_j}} \cup \{ (x^{(a_j, k)} ; a_j), (y^{(a_j, k)} ; a_j) \mid k \in K_j\}
\]
is easily seen to be unobstructed.
We can ask for more:

\begin{claim}\label{claim:half-cog-freshened-up}
    We may choose $x^{(a_j, k)}$'s, and $y^{(a_j, k)}$'s so that %the union $\bigcup_{j=1}^mV_j$
    \[
    	V_0 = \vsup{v}, \qquad V_j = V_{j-1}\cup U_j
 %       V_j= \vsup{v} \cup \{ (x^{(a_{j'}, k)} ; a_{j'}), (y^{(a_{j'}, k)} ; a_{j'}) \mid k \in K_{j'}, 1 \leq j' \leq j\}
    \]
    is unobstructed for $j = 1, 2, \dots, m$.
\end{claim}
\begin{proof}
    Let $A_j$ denote the atoms that appear in $\vsup{v^{i^* : a_j}}$.
    Given any automorphism $\pi \in \Aut(\A)$ that fixes $S$ as well as $A_j$, 
    using the tuples $\pi \cdot x^{(a_j, k)}$ and $\pi \cdot y^{(a_j, k)}$ gives us another decomposition of $v^{i^* : a_j}|^{-i^*}$ that is unobstructed.
   ...
   % For $j\leq m$, assume that $U=\bigcup_{j'=1}^{n-1}V_{j'}$ is unobstructed.
    Assume that $V_{j-1}$ is unobstructed. Suppose that $V_j$ is not even unambiguous.
    Then for some $b\in V_j\setminus V_{j-1}\subseteq U_j$ and $c\in V_{j-1}$ have $b_i = c_{i'}$ with $i \neq i'$.
   %  where $b^{(k)}_\bullet$, say, is some $x^{(a_j, k)}_\bullet ; a_j$ or $y^{(a_j, k)}_\bullet ; a_j$.
    Now if we were to have $b_i \in A_j$ (e.g., if $b_i = a_j$), then $b_i = d_{i''}$ for some $d \in \vsup{v^{i^* : a_j}} \subseteq \vsup{v}$. Since $U_j$ is unambiguous, $i=i''$; and since $V_{j-1}$ is unambigous, $i''=i'$. This contradicts the assumption that $i\neq i'$.
%    But this is impossible: $\vsup{v}$ and $V_{j-1}$ are both unambigous, so $i=i''$ and $i''=i'$.
    
    So the ambiguous atom $b_i$ does not belong to $A_j$.
    In other words, the set $X$ of all atoms that appear in $V_j$ except $b_i$ contains $A_j$ (and $a_j$).
    Apply Lemma~\ref{lem:free-fresh} to get an automorphism $\tau$ which fixes $S \cup X$ but sends $b_i$ to a fresh atom.
    Then, in the new family 
    \[
      V_{j-1} \cup \{ (\tau \cdot x^{(a_{j}, k)} ; a_{j}), (\tau \cdot y^{(a_{j}, k)} ; a_{j}) \mid k \in K_{j}\}, 
    \]
    the ambiguous atoms are precisely the ones in the old family minus $b_i$ --- these are all fixed by $\tau$.
    We continue this way until all ambiguous atoms are freshened.

    Hence assume $V_j$ is unambiguous.
    Suppose $b_i$ and $c_{i'}$, for some $b,c\in V_j$, are the reason why $V_j$ fails to be unobstructed.
    To have $b_i$ in $A_j$ is impossible:
    otherwise $b_i = d_{i''}$ for some $d \in \vsup{v^{i^* : a_j}} \subseteq \vsup{v}$ and since $V_j$ is unambiguous, we
    get $i = i''$. But $d_i$ (hence $b_i$) cannot be part of an obstruction in $V_j$ because both $U_j$ and $V_{j-1}$ are unobstructed.
    Symmetrically, we see that $c_{i'} \not\in A_j$.

    Since $V_{j-1}$ is unobstructed, $b$ (or, symmetrically, $c$) must be one of the $x^{(a_j, k)};a_j$ or $y^{(a_j, k)};a_j$. 
   % Finally, notice that by the claim for $j - 1$, an obstruction $b^{(k)}_i$ must again come from $b^{(k)}_\bullet$ being some $x^{(a_j, k)}_\bullet ; a_j$ or $y^{(a_j, k)}_\bullet ; a_j$.
    This time, split all atoms that appear in $V_j$ except $b_i$ into two sets $X$ and $Y$:
    put any atom $c_{i'}$ which makes $b_i$ an obstruction in $Y$, and put everything else in $X$.
    Then $X$ contains $A_j$ as discussed above;
    it also contains $\{x^{(a_j, k)}_i, y^{(a_j, k)}_i \mid k \in K_j, i \in I\}$.
    Use Lemma~\ref{lem:free-fresh} to get $\tau \in \Aut(\A)_{(S \cup X)}$ which sends $b_i$ to a fresh atom disjoint from $S \cup X \cup Y \cup \{b_i\}$ that is unrelated to everything in $Y$.
    Observe that the new family
    \[
        V_{j-1} \cup \{ (\tau \cdot x^{(a_{j}, k)};a_{j}), (\tau \cdot y^{(a_{j}, k)};a_{j}) \mid k \in K_{j}\}, 
    \]
    remains unambiguous.
    Moreover, the obstructions here are precisely the ones from $V_j$ except the ones that involve $b_i$, which are all fixed by $\tau$.
    So we may repeat this process until all obstructions are removed.
\end{proof}

Having chosen the fresh atoms carefully, let $X$ consist of all the atoms appearing in the unobstructed family $V_m$. It will be convenient to fix a representative $o\in{\cal O}$ and recall the function 
\[
\rt{-}: X \to \{o_1,\ldots, o_d\}
\] 
introduced in Definition~\ref{def:unobstructed}.
We now add a new element $b^*$ to the finite $\mathcal{L}$-structure $S \cup X \subseteq \A$ so that $(x^{(a_j, k)} ; a_j)\parallel (y^{(a_j, k)} ; b^*)$ becomes a well-formed $\cal O$-duo for each $j$.  To this end we define $b^*$ to be greater (wrt. $>$) than every atom in $X$, and the remaining relations are defined to make $b^*$ mimic the greatest atom in $o$:
%We define the new relations in $T = S \cup X \cup \{b^*\}$ by
\[
\begin{aligned}
    b^* > c & \quad \text{ for } c\in X \\
     b^* > s &\iff \A \models o_{i^*} > s \\
    P(b^*) &\iff \A\models P(o_{i^*}) \\
    R(b^*, s) &\iff \A \models R(o_{i^*}, s) \\
   R(s, b^*) &\iff \A \models R(s, o_{i^*}) \\
    R(b^*, c) &\iff \A \models R(o_{i^*}, \rt{c}) \text{ for } c\in X\\
   R(c, b^*) &\iff \A \models R(\rt{c}, o_{i^*})  \text{ for } c\in X 
\end{aligned}
\]
for every unary predicate $P$ and binary relation $R$ in $\mathcal{L}_0$.
...
The last two clauses are well defined (i.e. they do not depend on the choice of $a\in V_m$ that contains the atom $c$) since $V_m$ is unambiguous.
%
%So far we only relied on the unambiguity of the family;
%next we use the full assumption that it is unobstructed:
%
Moreover, since $V_m$ is unobstructed, our definition does not introduce any forbidden substructures:
\begin{claim}
    $S \cup X \cup \{b^*\}$ embeds into $\A$.
\end{claim}
\begin{proof}
    If not, by Lemma~\ref{lem:free-forb} there is a forbidden $\mathcal{L}_0$-structure $F$ of pairwise related elements which embeds into the $\mathcal{L}_0$-reduct of $S \cup X \cup \{b^*\}$, via $\phi$ say.  Extend $\rt{-}$ from above to:
    \[
    \rt{-}: X\cup S\cup\{b^*\} \to \{o_1,\ldots, o_d\}\cup S
    \]
     by putting $\rt{b^*}=o_{i^*}$ and $\rt{s}=s$ for $s\in S$.
    We will check that $\sqrt{-} \circ \phi$ then embeds $F$ into the $\mathcal{L}_0$-reduct of $\A$, namely $\A_0$, and reach a contradiction.

    So let $P \in \mathcal{L}_0$ be unary and let $R \in \mathcal{L}_0$ be binary.
    It is immediate that $\rt{-}$ preserves and reflects $P$ for all atoms in $X\cup S$ and for $b^*$ --- we do not even need to know they are in the image of $\phi$.
    We can also see that $\rt{-}$ preserves and reflects $R$ whenever at least one of the arguments is from $S \cup \{b^*\}$.
    Now consider the remaining case of $\phi(f), \phi(f') \in X$. As $f$ and $f'$ are related in $F$, 
    their images under $\phi$ must be related in $\phi(F)$.
    But the family $V_m$ that gave rise to $X$ is unobstructed, which forces $\rt{-}$ to preserve and reflect $R$ here as well.
    Since $\mathcal{L}_0$ only has unary and binary relations, this means that $\sqrt{\phi(F)} \subseteq \A_0$ is in $\mathcal{C}_0$ yet embeds $F$; this is a contradiction.
\end{proof}

Using homogeneity we may assume that $S \cup X \cup \{b^*\} \subseteq \A$.
\begin{claim}
    $x^{(a_j, k)} ; a_j \parallel y^{(a_j, k)} ; b^*$ forms an $\mathcal{O}$-duo 
    for $1 \leq j \leq m$ and $k \in K_j$.
    Furthermore, the family \[
        \vsup{v} \cup \{ (x^{(a_j, k)} ; a_j),  (y^{(a_j, k)} ; b^*) \mid k \in K_j, 1 \leq j \leq m \} 
    \] is unobstructed.
\end{claim}
\begin{proof}
    Recall that $x^{(a_j, k)} \parallel y^{(a_j, k)}$ is a duo for $\mathcal{O}^{i^* : a_j}|^{-i^*}$. Since $x^{(a_j, k)},  y^{(a_j, k)}\in X$, the remaining conditions for $x^{(a_j, k)} ; a_j \parallel y^{(a_j, k)} ; b^*$ being an $\cal O$-duo follow directly from our definition of relations on $b^*$.
%    Given any $o \in \mathcal{O}$,
%    it remains to show that
%    \[
%       P(b^*) \leftrightarrow P(o_{i^*})
%    \]
%    for any unary $P \in \mathcal{L}_0$ and that
%    \begin{align*}
%        \A \models 
...
The family in the claim is unambiguous since $V_m$ is unambiguous and $b^*$ is a fresh atom. Similarly, it is unobstructed since $V_m$ is unobstructed, and the additional conditions that involve $b^*$ follow from the relations that we imposed on $b^*$.
...
\textcolor{red}{BK: This argument feels a bit shaky; I will think about it some more.}
\end{proof}

\begin{claim}
    We have $v = \sum_{j = 1}^m \sum_{k \in K_j} 
    v(b^{(a_j, k)} ; a_j) \cdot
    (x^{(a_j, k)} ; a_j \between y^{(a_j, k)} ; b^*)$.
\end{claim}
\begin{proof}
    Observe that, by definition of cogs, for any $\cal O$-duo $a;c\parallel b;d$ with $c,d\in\A$ we have
    \[
    	(a;c) \between (b;d) = (a \between b); c - (a\between b);d.
    \]
    Using this, calculate:
    \begin{align*}
        &\sum_{j=1}^m \sum_{k \in K_j} v(b^{(a_j, k)}) \cdot (x^{(a_j, k)} ; a_j \between y^{(a_j, k)} ; b^*) \\
        ={} &\sum_{j=1}^m \sum_{k \in K_j} \Bigl( v(b^{(a_j, k)}) \cdot x^{(a_j, k)} \between y^{(a_j, k)} \Bigr) ; a_j \\
            &-\sum_{j=1}^m \sum_{k \in K_j} \Bigl( v(b^{(a_j, k)}) \cdot x^{(a_j, k)} \between y^{(a_j, k)} \Bigr) ; b^* \\
        ={}& \sum_{j=1}^m v^{i^*: a_j} - \sum_{j=1}^m v^{i^* : a_j}|^{-i^*} ; b^* \\
        ={}& v - \sum_{j=1}^m v^{i^* : a_j}|^{-i^*} ; b^* ,
    \end{align*}
    and it suffices to show the last sum vanishes.
...
we have shown any balanced vector $v$ is spanned by cogs as long as $\vsup{v}$ is unobstructed.

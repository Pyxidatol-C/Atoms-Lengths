...
We want to consider every $k$ and $J \subseteq I_k$ that yields the same image $\cal Q$;
let $\Jclass$ consist of these pairs $(J, k)$.
We can group together the projection maps as follows:
\begin{align*}
    {\restriction} : \Lin_\FF X &\rightarrow \Lin_{\FF^\Jclass} \cal Q \\
    v {\restriction} (q)_{J, k} &= v_k |^J (q^J).
\end{align*}
In words, $v {\restriction}$ allows us to have local information around $q$ about the coefficients in $v$.

We do it for all such $\cal Q$.


\textcolor{red}{The column-fintie and equivariant restriction does not lose generality for the ordered atoms.}

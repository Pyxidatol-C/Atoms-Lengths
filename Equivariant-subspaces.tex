...
By this we mean the decision procedure: given $\mathrm{A}, \mathrm{b}$, does there exist a solution to $\mathrm{A} \mathrm{x} = \mathrm{b}$?
Here the rows and columns are orbit-finite, the matrix is equivariant, and in every column there are only finitely many non-zero entries.

...
\begin{example}
    [Vector space] \label{ex:vector-space-atoms} Fix some finite field, and let $\A$ be the vector space of \textcolor{blue}{countably infinite} dimension over this field. This vector space is seen as a structure over an infinite vocabulary, which contains a relation 
    \begin{align*}
    ...
    This particular example can lead to some confusion, since here $\A$ is itself a vector space, and we will later on consider the vector space (possibly over different fields) with $\A$ as a basis. %Hence, there will be two vector space structures under consideration. 
\end{example}

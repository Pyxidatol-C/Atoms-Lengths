...
We start with an oligomorphic structure $\A$, as described in Section~\ref{sec:structures}, whose elements we call \emph{atoms}. These can be used to construct sets that are finite up to the symmetries of $\A$, such as
\begin{align*}
...
Let us now explain how the above definition coincides with first-order interpretations. We can think of an orbit-finite set as a relational structure, by equipping it with some relations that are equivariant with respect to the action of the automorphism group of $\A$. \textcolor{red}{But we don't consider relations until the conclusion? (Also, mention orbit-finite sets = sorts in $\A^{eq}$, and orbit-finitely spanned vector spaces are finitely generated $\FF[\Aut(\A)]$-modules where the action is continuous under the Polish topology?)}
  Since we assume the atoms to be oligomorphic, the equivariant relations are exactly those that can be defined in first-order logic. Therefore, the above definition is the same as a first-order interpretation in $\A$. 

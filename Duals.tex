...
\begin{myexample}\label{ex:no-function-spaces}
    Assume that the $\A$ is the Rado atoms, and the field is arbitrary\footnote{A variant of this example for the two-element field was shown in~\cite[Ex. 6.9]{BFKM24}, but here we can use any field, in particular a field of characteristic zero as treated in the previous section.}. (The example would also work for the Hensen graph that will be defined later.)  
    Consider the space 

...
They are all, as we will explain, homogeneous and, thanks to the restriction on the vocabulary, oligomorphic.
...
\begin{example}[Equality atoms] \label{ex:equality-atoms} 
    ...
    Two tuples are in the same orbit if and only if they have the same equality pattern. 
    Since there are finitely many equality patterns for tuples of fixed length, this structure is oligomorphic. 
    For example, in dimension $d=2$ there are two orbits: $x_1=x_2$ and $x_1 \neq x_2$.
...
\begin{example}[Ordered atoms] \label{ex:order-atoms} 
    ...
    and two tuples are in the same orbit if and only if they have the same order pattern. 
    Since there are finitely many order patterns for tuples of fixed length, this structure is oligomorphic. 
...
\begin{example}[Vector atoms] \label{ex:vector-space-atoms} 
    ...
    This vector space is seen as a structure over an infinite vocabulary, which contains a relation 
    \begin{align*}

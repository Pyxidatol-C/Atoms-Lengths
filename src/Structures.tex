...
--- i.e.~$\{a_1, a_2\} \to \{b_1, b_2\}; a_1 \mapsto b_1, a_2 \mapsto b_2$ must be an isomorphism.
...
--- see for instance~\cite[Cl.~5.9]{bojanczyk_slightly}. 
...
Every homogeneous structure arises via a construction called the Fraïssé limit~\cite[Sec.~7.4]{hodges1993model}, from classes of finite structures that satisfy certain closure properties (the so-called amalgamation classes). 
...
\begin{example}[Rado atoms]\label{ex:rado-graph} The Rado graph is the Fraïssé limit of the class of finite undirected graphs. Here, an undirected graph is viewed as a structure that has one binary relation that is symmetric and irreflexive. 

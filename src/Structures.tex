...
\begin{example}[Vector atoms] \label{ex:vector-space-atoms} 
    ...
    This vector space is seen as a structure over an infinite vocabulary containing, for every $d \in \set{1,2,\dots}$ and all coefficients $\lambda_1, \dots, \lambda_d$ in $\innerfield$, the relation 
    \begin{align*}
    ...
    In $\A^2$ there are $2 + |k|$ more orbits:
    $x_1 = 0 = x_2$;
    $x_1 = 0 \neq x_2$;
...
\begin{example}[Rado atoms]\label{ex:rado-graph} 
    ...
    An explicit description of the Rado graph is as follows: its vertices are the natural numbers, and there is an edge between $n < m$ if the $n$-th least significant bit in the binary representation of $m$ is $1$.
    ...
    As a Fraïssé limit, the Rado graph is already homogeneous.

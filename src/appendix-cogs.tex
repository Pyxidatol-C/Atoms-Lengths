...
Recalling that $a$ and $b$ are related if they are equal, we note an equational conflict is a special case of a relational one.
...
\begin{lemma}\label{lem:?}
    Let $K, V_0 \subseteq V$ be finite subsets of $\cal O$ such that both $V_0\cup K$ and $V$ are free from equational conflicts. Then there exists a $\pi\in\Aut(\A)$ that, while fixing all atoms in $S$ and in $\sqrt{V_0}$, makes $V \cup \pi(K)$ free from equational conflicts.
\end{lemma}
\begin{proof}
    ...
    the inductive hypothesis gives us some $\pi' \in \Aut(\A / S \cup \sqrt{V_0})$ such that $V \cup \pi' \pi(K)$ is free from equational conflicts.
\end{proof}
...
\begin{proof}
    ...
    Since there are no conflicts in $V_0\cup K$ or in $V$, we see that $(i, a)$ is not a location in $V$ and $(j, b)$ is not a location in $V_0 \cup K$~---~i.e.~since there are no equational conflicts, 
...
The outer base case $I = \emptyset$ is trivial~---~we have $v = \lambda \cdot ( \between )$, and no conflicts arise~---~and the inner base case is just Claim~\ref{claim:!-free-decomposition}.

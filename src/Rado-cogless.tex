...
\end{figure}
   ...
   %     where the supremum ranges over finite sets of atoms.
    ...
    This will let us leverage an elementary result from representation theory called Maschke's Theorem (\textcolor{magenta}{see (TODO: find better source that doesn't use C)} e.g.~\cite[Ch.~8]{JamesLiebeck01}), 
    \textcolor{magenta}{which decomposes spaces into indecomposable parts.} 
    (In the theorem below, $\oplus$ stands for the direct sum of vector spaces, i.e.~$V \oplus W$ is the set of pairs $(v,w)$ with $v \in V$ and $w \in W$, where the operations are defined coordinate-wise.)
    ...
    (We do not bother to equip this space with the action of $G$, yet --- here it would be trivial.) 
    ...
    \begin{proof}
        ...
        The length is additive with respect to direct sums, i.e.
        \begin{align*}
            \text{length of $V$} 

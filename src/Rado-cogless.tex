...
Under this assumption, we will establish the finite length property for the Rado atoms (Example~\ref{ex:rado-graph}) and for the vector atoms (Example~\ref{ex:vector-space-atoms}). 
These are new results. 
We also think that the proof itself, even when applied to get already known results (Example~\ref{ex:equality-atoms}), is of independent interest and arguably simpler than previously known proofs.
...
\begin{definition}[Oligomorphic approximation]\label{def:oligomorphic-approximation} 
    ...
    \begin{bracketenumerate}
        ...
        every finite substructure of $\A$ is a substructure of some $\B \in \Bclass$; and
...
\begin{theorem}\label{thm:have-oligomorphic-approximation}
    ...
    \begin{romanenumerate}
        ...
        \textcolor{magenta}{smoothly approximated structures in the canonical vocabularies.}
    \end{romanenumerate}
\end{theorem}
...
\begin{corollary}\label{cor:weak-finite-length}
    ...
    (\ref{item:rado-has-oligomorphic-approximation}) the Rado atoms; and 
    (\ref{item:smooth-implies-oligomorphic-approximation}) smoothly approximated structures.
\end{corollary}

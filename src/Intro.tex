...
If we choose the atoms to be the Fraïssé limit of total orders, as in row~2 of Figure~\ref{fig:fraisse-limits}, then the orbit-finite version of \textsc{p}$=$\textsc{np} has the same answer as the classical version without atoms. 
...
\begin{itemize}
    ...
    One is to exhibit an AZ enumeration \cite[Thm.~2.1]{RationalsHaveAZ} and derive the ``nice orderings''~\cite[p.~7]{Evans_pm2}.
...
In fact, Evans~\cite[Rem.~1.3]{Evans_pm1} observed that for any Fraïssé limit over a finite relational vocabulary, failing to have the finite length property over some finite field

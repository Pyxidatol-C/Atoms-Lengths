...
\begin{definition}[Orbit-finite-dimensional vector space]\label{def:orbit-finite-basis}
    $\Lin_\field X$ is, given an orbit-finite set $X$ and a field $\field$,Žthe vector space of finite formal linear combinations of elements in $X$ over $\field$.
\end{definition}
...
\lipicsEnd\end{example}
 ...
 For this reason, below we present a more general notion of a vector space, in a style that emphasises the group action as was done in Definition~\ref{def:orbit-finite-set-alternative}.
...
The spaces defined above certainly include those of the form $\Lin_\field \A^d$, as $\A^d$ is orbit-finite. 
They are easily seen to be closed under taking quotients by equivariant subspaces. 
...
In light of Theorem~\ref{thm:ACP} and the proof of (\ref{item:LinAd-is-Noetherian}) $\Rightarrow$ (\ref{item:ofs-implies-Noetherian}) in Appendix~\ref{sec:appendix-modules}, we could have used $\A^d$ instead of $X$ in the above definition.
Like mentioned in the introduction, the finite length property can be strictly stronger than the ascending chain property, as witnessed by $\Lin_\field \A$ over the vector atoms from Example~\ref{ex:vector-space-atoms}. 

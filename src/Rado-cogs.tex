...
\begin{description}
    ...
    Assuming that $\Cclass_0$ is closed under substructures and isomorphisms, 
...
\begin{example}\label{ex:free-amalg-graphs}
	...
	&
	\qquad\xymatrix@C=15pt{\bigdot&\bigdot}
	&
	...
	\quad\sf A & \quad~ \sf B & \sf C & \sf D & \sf E
	\end{array}
    \]
    %
%    
...
%    where $E$ is interpreted respectively as $\{(0, 0)\}$, $\{\}$, $\{(3, 4)\}$, $\{(5, 6), (6, 5)\}$, and $\{(7, 8), (8, 7), (8, 9), (9, 8), (9, 7), (7, 9)\}$.
    ...
    \begin{itemize}
        \item 
        $\Forb({\sf A}, {\sf C}\})$ is a free amalgamation class and consists of all finite graphs;
        
        \item
        $\Forb(\{{\sf A}, {\sf C},{\sf E}\})$ is a free amalgamation class and consists of all finite triangle-free graphs;
        
        \item 
        $\Forb(\{{\sf A}, {\sf D}\})$ is a free amalgamation class and consists of all finite digraphs;

        \item 
        $\Forb(\{{\sf A}, {\sf B},{\sf D}\}\})$ consists of all finite tournaments. 
        It is an amalgamation class,
        but not a free one: 
        in the minimal forbidden structure $\sf B$, the two elements are not related.
    \end{itemize}
\lipicsEnd\end{example}
...
\begin{remark}
    ...
    A positive answer is known for the equality and the ordered atoms~\cite[Thm.~4.10]{BFKM24},
...
Before state this (as Theorem~\ref{thm:cog-span-generally}), let us illustrate the key ideas of a proof on an example.

...
\begin{description}
    ...
    (This does not lose generality, since we can consider $R_{\neq}(x,y)$ and $R_=(x)$ separately: see~\cite[Sec.~2.4]{Herwig1998} or~\cite[p.~121]{SS20}.)
...
\begin{remark}
    ...
    but the argument relies on a property that they have~\cite[Lem~2.21]{BodirskyBodorMarimon_25} and the Rado atoms lack~\cite{no-constant-interpreted-in-Rado}.
...
\begin{example}\label{ex:generically-ordered-graphs}
    ...
    The ordered Rado graph was studied in~\cite{BPP15} along with its first-order reducts (e.g.~the Fraïssé limit of all finite tournaments). The ordered triangle-free  Henson graph will be studied in an extended example in Section~\ref{ex:extended-example}.
\lipicsEnd\end{example}
...
but with significant new complications arising from the presence of nontrivial relations in $\A_0$.
...
It would be remiss to not address the fact that so far, we have not shown that $\cal O$-duos and $\cal O$-cogs even exist in general.
...
Let $\A_0$ be the universal triangle-free (undirected) graph and $\A$ its generically totally ordered version. 
...
(Such a new atom is not necessary if $\A$ is the ordered atoms, as we can simply take $z$ to be the largest atom present in the vector; 
...
Specifically, we postulate edges \[
    E(g', h), E(g', i), E(b', f), E(b', i)
\] and no more. 
...
Inspired by the example, we shall now assert, in greater generality still:

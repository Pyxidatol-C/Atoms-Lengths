...
\begin{description}
    ...
    Instead, we give a useful characterisation~\cite[Lem.~4.5]{SS20} of when the amalgamation class $\Cclass_0$ is free.
...
\begin{example}\label{ex:free-amalg-graphs}
    ...
    For instance, the ordered atoms is a tournament if we interpret $E$ as the order relation.

    Consider $\sigma_0$-structures ${\circlearrowright} = \{0\}$, ${\nrightarrow} = \{1, 2\}$, ${\rightarrow} = \{3, 4\}$, ${\rightleftarrows} = \{5, 6\}$, and $\triangle = \{7, 8, 9\}$, 
    where $E$ is interpreted respectively as $\{(0, 0)\}$, $\{\}$, $\{(3, 4)\}$, $\{(5, 6), (6, 5)\}$, and $\{(7, 8), (8, 7), (8, 9), (9, 8), (9, 7), (7, 9)\}$.
    Then:
    \begin{itemize}
        \item 
        $\Forb(\{{\circlearrowright}, {\rightarrow}\})$ is a free amalgamation class and consists of all finite graphs;
        
        \item
        $\Forb(\{{\circlearrowright}, {\rightarrow}, {\triangle}\})$ is a free amalgamation class and consists of all finite triangle-free graphs;
        
        \item 
        $\Forb(\{{\circlearrowright}, {\rightleftarrows}\})$ is a free amalgamation class and consists of all digraphs;

        \item 
        $\Forb(\{{\circlearrowright},  {\rightleftarrows}, {\nrightarrow}\})$ consists of all finite tournaments. 
        It turns out to be an amalgamation class,
        but not a free one: 
        in the minimal forbidden structure $\nrightarrow$, $1$ and $2$ are not related.
    \end{itemize}
\lipicsEnd\end{example}

\begin{description}
    \item[Irreflexivity] 
    For technical reasons, we restrict attention to classes $\Cclass_0$ where all structures are {\em irreflexive}, i.e. such that if $R(x,y)$ then $x\neq y$ for each relation $R$.
    % This does not lose generality: for any free amalgamation class $\Cclass_0$, we may replace every binary relation $R$ in the vocabulary with a new unary predicate $R_=$, 
    ...
    (Because of the total order, this amalgamation in $\Cclass$ is not free except in the trivial cases.)
...
For clarity, we give a terse summary of the setting:
\begin{theorem}\label{thm:ordered-free-amalg-has-finite-length}
    ...
    \end{itemize}
    Then the totally ordered structure $\A$, even with finitely many constants fixed (i.e.~added as unary predicates to the vocabulary), has the finite length property over any field.
\end{theorem}

\begin{remark}
    We do not know how to drop the arity restriction from our proofs, 
    so it is interesting that Evans quite recently developed a Ramsey-theoretic approach in~\cite{Evans_pm1} which could be used to show that $\Lin_\field \A^2$, 
    for free amalgamation classes over vocabularies of arbitrarily high arity, has finite length.
    It is unclear how to combine these results.

    We also do not know whether the finite length property of $\A$ implies the finite length property of $(\A, {=}a_1, \dots, {=}a_k)$ in general.
    A positive answer was known for the equality and the ordered atoms~\cite[Thm.~4.10]{BFKM24} previously,
    but the argument relies on a property that they have~\cite[Lem~2.21]{BodirskyBodorMarimon_25} but the Rado atoms do not~\cite{no-constant-interpreted-in-Rado}.
...
\begin{example}\label{ex:generically-ordered-graphs}
    Continuing from Example~\ref{ex:free-amalg-graphs}, we obtain generically ordered expansions of the Rado graph and of the \emph{triangle-free Henson graph}.
    The ordered Rado graph was studied in~\cite{BPP15} along with its first-order reducts (e.g.~the universal homogeneous tournament). The ordered triangle-free  Henson graph will be studied in an extended example in Section~\ref{ex:extended-example}.
\lipicsEnd\end{example}

So we arrive at the following general result:
\begin{corollary}\label{cor:lachlan-woodrow}
    ...
    \begin{romanenumerate}
        ...
        \item all of the countably many \cite{LachlanWoodrow_80} countable homogeneous graphs, including the Rado graph and the triangle-free Henson graph;
...
To prove Theorem~\ref{thm:ordered-free-amalg-has-finite-length} we will proceed in several steps, with the general idea similar to~\cite[Sec.~4.1]{BFKM24}, but with significant new complications arising from the presence of nontrivial relations in $\A_0$.
...
\begin{claimproof}
    ...
    \]
    which shows that $\Lin_\FF \cal O$ has finite length from the assumptions.
\end{claimproof}

As we will see shortly, there exist plenty of balanced vectors other than $0$.
...
\begin{theorem}[Compare with~{\cite[Cor.~3.17]{Gray97},~\cite[Thm.~15]{HofmanLerouxTotzke_17},~\cite[Thm.~3.4]{HofmanRozycki_2022},~\cite[Sec.~6]{GhoshHofmanLasota_22}}]\label{thm:coeff-approximation}
    ...
    \]
\end{theorem}
\textcolor{magenta}{We do not know how to test for equality.}

...
We shall now generalise that result to a wide class of structures. 
In particular, we will deduce the finite length property of the Rado atoms and its variants, even with finitely many constants fixed.

We shall now state our assumptions and results, with proofs in Section~\ref{sec:cogs-turn}.

Rewrite this introduction: except for $\Q$ and maybe the Rado graph, 
here it's easier to specify the age (define!) rather than the concrete structure.
...
This is a standard notion in model theory~---~see~\cite[Sec.~2.1]{Macpherson11}.
...
There is a useful characterisation~\cite[Lem.~4.5]{SS20} of the amalgamation class $\Cclass_0$ being free.
...
any two elements $x, y$ are either equal or satisfy at least one of $R(x, y)$ and $R(y, x)$ for some binary relation $R \in \sigma_0$~---~from here on, we will just say $x, y$ are \emph{related}.
...
\begin{example}\label{ex:free-amalg-graphs}
    ...
    \end{align*}    
    are all free amalgamation classes~---~notice that in each forbidden structure, any two elements are related. 
\lipicsEnd\end{example}

\paragraph*{Irreflexivity} 
For technical reasons, we restrict attention to classes $\Cclass_0$ where all structures are {\em irreflexive} (or \emph{injective}), i.e. such that if $R(x,y)$ then $x\neq y$ for each relation $R$. This does not lose generality: for any free amalgamation class $\Cclass_0$, we may replace every binary relation $R$ in the vocabulary with a new unary predicate $R_=$, interpreted in every structure so that $R_=(x)$ iff $R(x,x)$, and a binary relation interpreted as the irreflexive part of $R$. Then $\Cclass_0$ interpreted over this modified vocabulary is still a free amalgamation class, and all our results transport back to the original $\Cclass_0$. This way of encoding arbitrary structures as irreflexive ones is standard; see e.g.~\cite[Sec.~2.4]{Herwig1998} and~\cite[p.~121]{SS20}. All examples considered here are already irreflexive.
...
For clarity, we begin with a terse restatement of the setting.

%Theorem 7.3. Please write a standalone readable version of this theorem. 
%Something like: “Let A_0 be a homogeneous relational structure in a binary language L such that Age(A_0) has free amalgamation. 
%Let A_1 be a homogeneous structure in binary Language L disjoint union {<} such that Age(A_1) consists of all total orderings of Age(A_0)-structures where {<} is expressed to describe the total ordering. 
%Then, A_1 has the finite length property over a field.” 
%Feel free to write your own version instead or add assumptions if I have missed some.
...
\begin{theorem}\label{thm:ordered-free-amalg-has-finite-length}
    ...
    \end{itemize}
    Then the totally ordered structure $\A$, even with finitely many constants fixed, has the finite length property over any field.
\end{theorem}

\textcolor{magenta}{(I) We do not know how to drop the arity restriction from our proofs, 
so it is interesting that recently, using a Ramsey-theoretic approach, 
Evans proved~\cite[Prop.~3.10]{Evans_pm1} that $\Lin \A^2$ has finite length for similar structures with vocabularies of arbitrarily high arity. 
We do not know how to combine these results.
(II) we don't get constants for free: in \cite{BFKM24} we got lucky, since the equality atoms and the ordered atoms eliminate constants.}


Before we prove this theorem in Section~\ref{sec:cogs-turn}, let us state an easy consequence of it and give some examples.
The Fraïssé limit $\A_0$ of $\Cclass_0$ is a reduct of $\A$.
To see this, notice that $\A$, when viewed as an $\sigma_0$-structure, embeds the same finite $\sigma_0$-structures as $\A_0$~---~namely, $\Cclass_0$.
Moreover, it follows from a back-and-forth argument~\cite[Lem.~7.1.4]{hodges1993model} that the $\sigma_0$-structure $\A$ is also homogeneous and, therefore, that it is isomorphic to $\A_0$.
...
\end{corollary}
\begin{proof}
    A chain of subspaces in $\Lin_\FF \A_0^d = \Lin_\FF \A^d$ each supported by $S \subseteq \A_0$ is also a chain of subspaces supported by $S \subseteq \A$; 
...
\begin{example}\label{ex:generically-ordered-graphs}
    Continuing from Example~\ref{ex:free-amalg-graphs}, we obtain generically ordered expansions of the Rado graph and of the $K_n$-free versions of it, which are called \emph{Henson graphs}.
    The ordered Rado graph was studied in~\cite{BPP15} along with its first-order reducts (e.g.~the universal homogeneous tournament). The triangle-free  Henson graph will be studied in an extended example in Section~\ref{ex:extended-example}.
\lipicsEnd\end{example}

\textcolor{magenta}{Reviewer A: Theorem 7.3/Cor. 7.4 has more corollaries.}
%Fell free to add them: The following structures have the finite length property over any field: 
%1) The random oriented tournament. That is the graph with vertices IN, 
% such that between x and y is (if not x=y) an edge x→y or an edge y→x which is decided at random,
% similar to the random graph. It is a reduct of the oritended random graph.
%2) The total order with a dense+codence unary predicate. 
% (Direct application where A_0 has a unary predicate.)
% 3) The tournament graph which does not embed ({1,2,3,4},{(12),(13),(14),(23),(34),(42)}) 
% and ({1,2,3,4},{(21),(31),(41),(32),(43),(24)}) 
% as it is a reduct of the total order with a dense+codence unary predicate: 
% This structure can be realized as the rational numbers with a dense+codense subset S 
% where there is an edge x→y iff x<y and they are both in S, x<y and 
% they are both not in S or x>y and exactly one of them is in S.

Using a classification result of Lachlan and Woodrow~\cite{LachlanWoodrow_80}, we can get the following general result.

\begin{corollary}\label{cor:lachlan-woodrow}
    The following structures have the finite length property over any field:
    \color{magenta}
    \begin{romanenumerate}
        \item the ordered atoms;
        \item the equality atoms;
        \item the Rado atoms;
        \item all countable homogeneous graphs;
        \item all homogeneous tournaments (universal one in~\cite{BPP15}, dense local order in~\cite{NguyenVanThe13});
        \item uncountably many digraphs obtained by forbidding tournaments (cite Macpherson)
    \end{romanenumerate}
    Every countable homogeneous undirected graph has the finite length property over any field.

    Mention the vector atoms / universal partial order are not free?
\end{corollary}
...
We will proceed in several steps, with the general idea similar to~\cite[Sec.~4.1]{BFKM24}, but with significant new complications arising from the presence of nontrivial relations in $\A_0$.
...
To start with, let us view $\A^d$ as $\A^{\{1, \dots, d\}}$. More generally, it will be convenient to consider $\A^I$ for a finite totally ordered indexing set $I$ (say contained in $\Q$, so that we may take unions later).
...
$\cal O = \Aut(\A/S) \cdot a$ only contains $S$-ordered tuples, and we shall call the orbit $S$-ordered as well.%
\footnote{Here and in the following, $\Aut(\A/S)$ is the group of those automorphisms of $\A$ that fix every element of $S$. So ``$\Aut(\A/S)$-equivariant'' is synonymous with ``$S$-supported''.}
If $a$ is not $S$-ordered, by removing the entries that repeat or come from $S$ and reordering the rest, we can always find an $\Aut(\A/S)$-equivariant bijection from $\cal O$ to an $S$-ordered orbit.
...
\begin{claim}\label{claim:reduction-to-one-orbit}
    \color{magenta}
    The following are equivalent for every finite $S \subseteq \A$ (where $\A$ is oligomorphic):
    \begin{alphaenumerate}
        \item For each $d \in \{1, 2, \dots\}$, there is some finite bound $l_d$ such that chains of $\Aut(\A/S)$-equivariant subspaces in $\Lin_\FF \A^d$ have length at most $l_d$;
        \item $\Lin_\FF \cal O$ has finite length with respect to $\Aut(\A/S)$ for every $S$-ordered orbit $\cal O$ (of every $\A^{d'}$).
    \end{alphaenumerate}
\end{claim}
\begin{claimproof}
    \color{magenta}
    ...
    Since $\A$ is oligomorphic, given any $S$ and $d$, 
...
So fix an $S$-ordered orbit $\cal O = \Aut(\A/S) \cdot o \subseteq \A^I$.
...
\textcolor{magenta}{the \emph{projection kernel}~---~only call it balanced?} is the $\Aut(\A/S)$-equivariant subspace of $\Lin_\FF \cal O$ below:
\[
    \Ker_\FF \cal O = \bigcap_{i \in I} \ker{ (-)|^{-i} }.
\]

\begin{claim}\label{claim:reduction-to-kernel}
    The following are equivalent for every finite $S \subseteq \A$:
    \begin{alphaenumerate}
...
Following the terminology used in~\cite[Eq.~(4)]{BFKM24}, we shall call a vector from the projection kernel \emph{balanced}.
...
For an obvious example, for any $S$-ordered $a\in\A^I$ and $J\subseteq I$, we have $a|^{I\setminus J}a|^J=a$.
...
For the special case of the ordered atoms (Examples~\ref{ex:order-atoms} and~\ref{ex:generically-ordered-equality}) the following construction was studied in~\cite[p.~11]{BFKM24}, and it already appeared earlier in~\cite[p.~125]{Gray97} under the name of ``polytabloids''.
...
%     \] 
%     is a duo for $\cal O_k = \Aut(\A)_{(S \cup \{a_{i_{k+1}}, \dots, a_{i_d}\})} \cdot a|^{\{i_1, \dots, i_k\}}$~---~note that $() \parallel ()$ is certainly a duo for $\cal O_0$ at the start.
...
In light of Claims~\ref{claim:reduction-to-one-orbit},~\ref{claim:reduction-to-kernel} and~\ref{claim:cogs-are-balanced}, for the finite length property it is enough to prove that $\Ker_\FF \cal O \subseteq \Cog_\FF \cal O$. In words, we need to show that every balanced vector in $\Lin_\FF{\cal O}$ is a linear combination of $\cal O$-cogs.
...
(Such a new atom is not necessary if $\A$ is the ordered atoms, as we can simply take $z$ to be the largest atom present in the vector: cf.~the proof of~\cite[\text{Cl.~4.7}]{BFKM24}.)
...
\begin{theorem}\label{thm:cog-span-generally}
    ...
    where $\EE$ is a subspace of some $\FF^n$.
\end{theorem}

\begin{remark}\label{rem:FF-EE}
We must tread carefully here, as $\EE$ is not a field.
(E.g.~$\EE$ is $\{ (\kappa, -\kappa) \mid \kappa \in \FF \}$ in $\FF^2$. 
...
\[
    \lambda \cdot a
\]
as a formal expression for an element of $\Lin_\EE \cal O$, instead of the result of a scalar multiplication.
...
Leaving the proof to Appendix~\ref{sec:appendix-cogs}, 
we have~---~as discussed at the end of Section~\ref{subsec:finding-cogs}~---~established Theorem~\ref{thm:ordered-free-amalg-has-finite-length}, by taking $\EE = \FF$.
But by allowing $\EE$ to be finite-dimensional vector spaces, we can now describe all equivariant subspaces: 
we need only look at local sums of coefficients.
(Stated in a another way, there are no interesting subspaces here~---~
...
\begin{theorem}\label{thm:coeff-approximation}
    ...
    \]
\end{theorem}
We do not know how to test for equality.
...
\begin{example}
    ...
    \]
    For an equivariant vector space $V \subseteq \Lin_\FF \A$, the subspace $V {\restriction_1}(\A) \subseteq \FF$ is either $\{0\}$ or the entire $\FF$.
    \begin{itemize}
        \item In the first case, $V$ must be the zero space.
        \item Otherwise, we similarly distinguish two cases for $V {\restriction_2}() \subseteq \FF$:
        \begin{itemize}
            \item if it is the entire $\FF$, then $V$ must be the full space $\Lin_\FF \A$ by Theorem~\ref{thm:coeff-approximation};
            \item if it is $\{0\}$, then $V$ must be the zero-sum space spanned by $\{a - b \mid a, b \in \A\}$ again using Theorem~\ref{thm:coeff-approximation}.
        \end{itemize}
    \end{itemize}
    So $\Lin \A$ has the same structure as the space over the equality atoms in Example~\ref{ex:length-one-dim-one}.

    For $d = 2$, there are three maps, one of which is the form $\restriction_2 : \Lin_\FF \A^2 \to \Lin_{\FF^5} \A$. 
    By considering subspaces of $\FF^5$, when the field is infinite we can exhibit infinitely many equivariant subspaces of $\Lin_\FF \A^2$.
    Note that $\Lin_\FF \A^2$ has finite length nonetheless (equal to $2^1 + 2^2 + 2^2$, by below).
\lipicsEnd\end{example}

\textcolor{magenta}{there are redundancies, so the thm only gives an upper bound. Luckily we find a chain of matching length.}
...
Variants of Theorem~\ref{thm:coeff-approximation} have been shown over the equality and the ordered atoms: 
see~\cite[Cor.~3.17]{Gray97},~\cite[Thm.~15]{HofmanLerouxTotzke_17},~\cite[Thm.~3.4]{HofmanRozycki_2022},~\cite[Sec.~6]{GhoshHofmanLasota_22}, and~\cite[Thm.~19]{Arka_notes}.
As described in~\cite[Sec.~9]{GhoshLasota_24}, such results allow us to decide the solvability of orbit-finite systems of linear equations. 
We briefly repeat the explanation.
...
\color{magenta}In the orbit-finite setting, we assume that the rows and columns are indexed by $\A^d$, that every column has finitely many non-zero entries, and that $j \mapsto \mathbf{A}_{-, j}$ is equivariant.
That is, we ask whether $\mathbf{b} \in \Lin_\FF \A^d$ is in the span of $\mathbf{A}_{-, j}$. 

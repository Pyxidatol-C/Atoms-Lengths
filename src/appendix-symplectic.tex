...
\section{Proof of Theorem~\ref{thm:have-oligomorphic-approximation}(\ref{item:rado-has-oligomorphic-approximation})}\label{sec:appendix-symplectic}

The symplectic vector spaces are used as a basic building block in~\cite{KLM89}.
Described as a classical geometry, they can indeed be found in classical textbooks such as~\cite[pp.~114--121]{Artin_symplectic} and~\cite[pp.~75--83]{Aschbacher_symplectic},
though it is often not clear whether we assume finite dimension or characteristic zero for the results we require. 
As symplectic vector spaces are an important and interesting structure to understand, we provide a self-contained exposition here, taking a straight-line path to the oligomorphic approximation of the Rado graph in Theorem~\ref{thm:appendix-rado-oligo-approx}.

\subsection{Symplectic vector spaces}
Let $\ff$ denote a finite field.
\begin{definition}
    ...
    Model-theoretically, we start with the same vocabulary as for the vector atoms (Example~\ref{ex:vector-space-atoms}) and add a binary predicate for every element of $\ff$.
\end{definition}
...
We will refer to vectors satisfying \eqref{eq:symplectic-basis} as a \emph{symplectic subbasis}. 
As the name suggests, such vectors must be linearly independent:
...
\begin{proposition}\label{prop:symplectic-basis}
    ...
    Then any finite symplectic subbasis \[
        e_1, \ldots, e_n, f_1, \ldots, f_n
    \] 
    can be extended to a symplectic subbasis that spans the whole $\W$ (i.e.~a symplectic basis).
\end{proposition}
\begin{proof}
    Suppose that $e_1, \ldots, e_n, f_1, \ldots, f_n$ do not already span $\W$;
    ...
    By the non-degeneracy of $\omega$, there is~---~rescaling if necessary~---~some $w$ such that $\omega(e_{n+1}, w) = 1$. 
    Now define
    \[
        f_{n+1} = w - \sum_{i=1}^n \omega(e_i, w) f_i + \sum_{i=1}^n \omega(f_i, w) e_i
    \]
    in a similar manner, 
    making $e_1, \ldots, e_n, e_{n+1}, f_1, \ldots, f_n, f_{n+1}$ a symplectic subbasis that spans $v$.
    We go through every element of $\W$ by continuing this way.
\end{proof}

We may even forget some pairs.
\begin{proposition}\label{prop:symplectic-basis-and-a-half}\textcolor{gray}
    ...
    Then we can find the missing $f_{n+1}, \ldots, f_{n+k}$ to form a symplectic subbasis.
\end{proposition}
\color{black}

\begin{proof}
    Suppose we have found $f_{n+1}, \ldots, f_{n+i}$ already such that
    \begin{align*}
        &e_1, \ldots, e_n, e_{n+1}, \ldots e_{n+i}, e_{n+i+1}, e_{n+i+2}, \ldots, e_{n+k},\\ 
        &f_1, \ldots, f_n, f_{n+1}, \ldots, f_{n+i}
    \end{align*}
    satisfy \eqref{eq:symplectic-basis}.
    Notice these vectors are linearly independent:
    in a linear combination that sums to $0$,
    the coefficients of $e_1, f_1, \dots, e_{n+i}, f_{n+i}$ must be zero,
    and we assumed that $e_{n+i+1}, \dots, e_{n+k}$ are linearly independent.
    By extending these to a basis $B$ of $\W$, we may define a linear function 
    \[
        \psi : \W \to \ff
    \]
    which sends $e_{n+i+1}$ to $1$ but every other $b \in B$ to $0$.
    Now apply Proposition~\ref{prop:symplectic-basis} to obtain a symplectic basis $e'_1, f'_1, \dots, e'_m, f'_m$ of $\W$, and put
    \[
        f_{n+i+1} = \sum_{j=1}^m \psi(e'_j) f'_j - \psi(f'_j) e'_j;
    \]
    then $\omega(-, f_{n+i+1})$ agrees with $\psi$ on this symplectic basis, so by linearity they must be the same function.
    In particular 
    \[
        \omega(e_{n+i+1}, f_{n+i+1}) = \psi(e_{n+i+1}) = 1,
    \]
    whereas $\psi(e_1), \dots, \psi(e_{n+k}), \psi(f_1), \dots, \psi(f_{n+i})$ are all $0$.
    Thus we have \eqref{eq:symplectic-basis} as required.
\end{proof}
...
\]
defines an isometric linear bijection $\langle e_i, f_i \rangle \to \langle e'_j, f'_j \rangle$.
\end{lemma}

It then follows from Proposition~\ref{prop:symplectic-basis} that, 
up to isometric linear bijections, 
...
we will opt for another approach that establishes smooth approximation and gives an explicit orbit count.
\textcolor{magenta}{Mention homogeneity too.}

\textcolor{magenta}{Explain automorphism, and mention we treat $\omega$ as two binary relations.}
...
\begin{proposition}[Witt Extension] \label{prop:symplectic-witt-extension}
    ...
    can be extended to an isometric linear bijection of $\W_n$ to itself (i.e.~an automorphism),
    and in turn to one of $\W_\infty$ to itself.
\end{proposition}
...
\begin{proof}
    ...
    $y_1, \ldots, y_k$ and $y'_1, \ldots, y'_k$ and form two symplectic subbases~---~call them $\mathcal{B}$ and $\mathcal{B}'$.
    They are of the same size.

    Now, by using Proposition~\ref{prop:symplectic-basis}, extend $\mathcal{B}$ and $\mathcal{B}'$ to symplectic subbases $\mathcal{C}$ and $\mathcal{C}'$ that span $\W_n$.
    ...
    \textcolor{magenta}{just call them automorphisms??}
    To finish, notice that $\mathcal{C}, e_{n+1}, \ldots, f_{n+1}, \ldots$ form a symplectic subbasis spanning $\W_\infty$, as do $\mathcal{C}', e_{n+1}, \ldots, f_{n+1}, \ldots$.
...
\begin{proposition}\label{prop:symplectic-oligomorphic}
    ...
    % OEIS A006116 if "q = 1", for bit-vector atoms.
\end{proposition}
\begin{remark}
    To anticipate the next subsection, we note a similarity with the Rado graph: 
    in $\G^k$ there are \[
        \sum_{d=0}^k \left\{\begin{matrix}k \\ d \end{matrix}\right\} \cdot 2^{\binom{d}{2}}
        \geq 
        \sum_{d = 0}^k \begin{bmatrix}k \\ d \end{bmatrix}_2 \cdot 2^{\binom{d}{2}}
    \] orbits,
    where the curly brackets count the number equivalence relations with $d$ classes on a $k$-element set.
\end{remark}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        such that $v_i$ is not spanned by $v_1, \ldots, v_{i-1}$~---~so we inductively ensure that $$\{v_{i'} \mid i' \in I, i' \leq i\}$$ is a basis for $\langle v_1, \ldots, v_i \rangle$;
    ...
    \end{enumerate}
    If $\pi : \W_\infty \to \W_\infty$ is an isometric linear bijection,
    ...
    and that it may be extended to an isometric linear automorphism $\pi$ of $\W_\infty$ by Proposition~\ref{prop:symplectic-witt-extension}.
    ...
    \]
    for $i \in I$ and $v_j = \sum_{i \in I, i < j} \Lambda_j(i) v_i$ for $j \not\in I$.
...
As smooth approximation is a special case of oligomorphic approximation, 
by Theorem~\ref{thm:weak-smooth-approximation-finite-length} we establish the finite length property of yet another \textcolor{magenta}{family} of structures:
\begin{corollary}
...
Let us finish the proof of Theorem~\ref{thm:have-oligomorphic-approximation}(\ref{item:rado-has-oligomorphic-approximation}).
Here, let $\ff$ be the two-element field.
\begin{definition}
    For $n = 0, 1, 2, \ldots$, the \emph{symplectic graph} $\widetilde \W_n$ has vertices $\W_n$ and edges
    \[
        E(v_1, v_2) \;:\Longleftrightarrow\; \omega(v_1, v_2) = 1.
    \]
...
\end{proposition}
\begin{proof}
    Clearly each isometric linear bijection of $\W_n$ is a graph automorphism of $\widetilde \W_n$.
    Conversely, every graph automorphism $f \in \Aut(\widetilde \W_n)$ is evidently isometric.
...
So the number of orbits in $\widetilde \W_n^k$ is precisely equal to the number of orbits in $\W_n^k$~---~in particular, it is bounded above by $\sum_{d=0}^k \begin{bsmallmatrix}k \\ d\end{bsmallmatrix}_2 \cdot 2^{\binom{d}{2}}$ independently of $n$ by Proposition~\ref{prop:symplectic-oligomorphic}.%
\footnote{This is the $k$-th term in the \href{https://oeis.org/A028361}{OEIS sequence A028361}.}
It remains to show $\widetilde \W_0 \subseteq \widetilde \W_1 \subseteq \widetilde \W_2 \subseteq \cdots$ embeds all finite graphs:

\begin{proposition}[{\cite[Thm.~8.11.2]{GR01}}]
    Every graph on at most $2n$ vertices embeds into $\widetilde \W_n$.
\end{proposition}
\begin{proof}[Proof (without graph-theoretic jargon)]
    ...
    So suppose $n \geq 1$ and $G$ has an edge $E(s, t)$.
    Let $G_{s,t}$ be the graph with vertices $G \setminus \{s, t\}$ and with edges $E_{s,t}$ that we will specify later.
    By induction, some embedding $f : G_{s, t} \to \widetilde \W_{n-1}$ exists.
    Define $f' : G \to \widetilde \W_n$ by
    \begin{align*}
        x \in G_{s, t} &\mapsto f(x) - \lceil E(x, s) \rfloor  f_n + \lceil E(x, t) \rfloor  e_n \\
        s &\mapsto e_n \\
        t &\mapsto f_n
    \end{align*}
    where $\lceil \Phi \rfloor $ is $1$ if $\Phi$ holds and $0$ otherwise.
    On one hand, we have
    \begin{align*}
        \omega(f'(x), f'(s)) &= \lceil E(x, s) \rfloor , \\
        \omega(f'(x), f'(t)) &= \lceil E(x, t) \rfloor 
    \end{align*}
    as desired.
    On the other,
    \begin{align*}
        \omega( f'(x_1), f'(x_2) ) 
        = \lceil E_{s,t}(x_1, x_2) \rfloor  
        &+ \lceil E(x_1, s) \rfloor  \lceil E(x_2, t) \rfloor  \\
        &- \lceil E(x_1, t) \rfloor  \lceil E(x_2, s) \rfloor 
    \end{align*}
    tells us how we should define the edge relation $E_{s,t}$ in $G_{s,t}$ for $f'$ to be an embedding of graphs, 
    i.e.~so that $\lceil E(x_1, x_2) \rfloor  = \omega( f'(x_1), f'(x_2) )$.
\end{proof}

Note that $\bigcup_{i} \widetilde \W_i$ is not isomorphic to the Rado graph.
For instance, there are no common neighbours of $e_i$, $f_i$, and $e_i + f_i$:
to have $1 = \omega(v, e_i + f_i) = \omega(v, e_i) + \omega(v, f_i) = 1 + 1$ is impossible.
Since $\bigcup_{i} \widetilde \W_i$ and the Rado graph share the same age, the homogeneity of the latter means two things:
\begin{bracketenumerate}
    \item $\bigcup_{i} \widetilde \W_i$ cannot be a homogeneous graph as well;
    \item nonetheless, we may consider the symplectic graphs $\widetilde \W_i$ as induced subgraphs of the Rado graph.
\end{bracketenumerate}

\begin{theorem} \label{thm:appendix-rado-oligo-approx}
    The Rado graph is oligomorphically approximated by the $\widetilde \W_i$'s.
\end{theorem}

\begin{corollary}
    The Rado graph has the finite length property over any field of characteristic zero.
\end{corollary}

A different finite field $\innerfield$ can encode different structures.
For example, the 3-element field allows us to prove the finite length property of the universal homogeneous digraph.\footnote{We are again grateful to Ehud Hrushovski for pointing this out.}
...
\begin{lemma}\label{lem:coeff-approximation}
    For $i = 1, \dots, t, t+1$,
    \begin{align*}
        &W \cap \ker(\restriction_i) \cap \ker(\restriction_{i+1}) \cap \cdots \cap \ker(\restriction_t) \\
        \subseteq{} &\widehat W \cap \ker(\restriction_i) \cap \ker(\restriction_{i+1}) \cap \cdots \cap \ker(\restriction_t).
    \end{align*}
...
\begin{claimproof}
    ...
    Apply Lemma~\ref{lem:cog-fresh-full} and Remark~\ref{rem:duo} to get automorphisms $\pi_j$ for $j \in J_i$ such that $a \parallel \prod_{j \in J_i} \pi_j a$ is an $\mathcal{Q}_i$-duo, 
...
For two equivariant subspaces $W, W' \subseteq \Lin_\FF \cal O$, 

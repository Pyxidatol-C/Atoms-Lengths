...
The hardest part of this proof is to set up the notation to define the maps $\{\restriction_i\}_i$.

\textcolor{red}{Fix $\biguplus_k \cal O_k$ with $\cal O_k \subseteq \A^{I_k}$. To minimise changes in the following, we write $J \subseteq I_k$ to mean $(J, k)$.}
...
The base case $i = 0$ is trivial, \textcolor{red}{since $\ker(\restriction_1) = \{0\}$.} Indeed, $\Jclass_1=\{I\}$, so $v{\restriction_1}$ is the identity map.
...
Now we specialise to the case where we have a single ordered orbit $\cal O \subseteq \A^d$.
...
\end{align*}
in $\Lin_\FF(\cal O)$. 
Let $\langle v_J \rangle$ be the linear span of permutations of $v_J$; this is an equivariant subspace.

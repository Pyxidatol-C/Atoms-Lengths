...
we can find $w \in W \cap \ker(\restriction_{i+1}) \cap \dots \cap \ker(\restriction_t) \subseteq \widehat W$ such that 
$v {\restriction_i} = w {\restriction_i}$.
But then $(v - w) {\restriction_{i}} = 0$, so $v - w$ lies in $\ker(\restriction_{i})$ as well as $\widehat W \cap \ker(\restriction_{i+1}) \cap \cdots \cap \ker(\restriction_t)$.
...
% Along the lines of Claim~\ref{claim:reduction-to-one-orbit}, this generalises to multi-orbit sets:
% \[

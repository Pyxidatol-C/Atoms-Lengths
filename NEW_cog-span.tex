...
\newcommand{\Yu}{\mbox{\usefont{T2A}{\rmdefault}{m}{n}\CYRYU}}
\newcommand{\Ya}{\mbox{\usefont{T2A}{\rmdefault}{m}{n}\CYRYA}}
\newcommand{\Che}{\mbox{\usefont{T2A}{\rmdefault}{m}{n}\CYRCH}}
\newcommand{\Tse}{\mbox{\usefont{T2A}{\rmdefault}{m}{n}\CYRC}}
\newcommand{\Sha}{\mbox{\usefont{T2A}{\rmdefault}{m}{n}\CYRSH}}
\newcommand{\che}{\mbox{\usefont{T2A}{\rmdefault}{m}{n}\cyrch}}
\newcommand{\tse}{\mbox{\usefont{T2A}{\rmdefault}{m}{n}\cyrc}}
\newcommand{\she}{\mbox{\usefont{T2A}{\rmdefault}{m}{n}\cyrsh}}
...
%TODO: equal elements are also \emph{related}. Does that matter?
...
Lastly, I refer to the atoms that appear in $\sigma$ by $\sqrt \sigma \subseteq \A$.
...
% \begin{Proof}
% \textcolor{red}{I have changed the statement: before it said $\Che \cup \Tse$ is unambiguous, as is $\Che \cup \Sha$.}
%     Fix $\Che, \Sha$ and proceed by induction on the size of $? \overline{\Che \cup \Tse \cup \Sha}$:
%     suppose we have $(i, a_i) ? (j, b_j)$.
%     It follows from the assumptions that $(i, a_i) \not\in \overline{\Che}$ and that without loss of generality $(i, a_i) \in \overline{\Tse}, (j, b_j) \in \overline{\Sha}$.
%     Put 
%     \[
%         X = S \cup \sqrt{\Che \cup \Tse \cup \Sha} \setminus \{a_i\}
%     \]
%     and note that $X$ contains $S \cup \sqrt{\Che}$.
%     Use the Hammer to obtain an automorphism $\pi \in \Aut(\A / X)$ so that $\pi(a_i) \not\in X \cup \{a_i\}$.
%     We may check that
%     \[
%         ?\overline{\Che \cup \pi(\Tse) \cup \Sha} \subseteq {?\overline{\Che \cup \Tse \cup \Sha}} \setminus \{(i, a_i)\}.
%     \]
%     Indeed, any ambiguity on the left must be between $(i', \pi(a'_{i'})) \in \overline{\pi(\Tse)}$ and $(k, c_k) \in \overline{\Sha}$.
%     We cannot have $a'_{i'} = a_i$, because then $\pi(a'_{i'}) = \pi(a_i)$ cannot be equal to $a_i = a'_{i'} = c_k$.
%     Hence $a'_{i'} \neq a_i$, and $(i', \pi(a'_{i'})) = (i', a'_{i'}) \in \overline{\Tse} \setminus \{(i, a_i)\}$ together with $(k, c_k) \in \overline{\Sha}$ was already an ambiguity on the right.
%     As \[
%         ?\overline{\Che \cup \pi(\Tse)} = {?}\overline{\pi(\Che \cup \Tse)} = \pi(?\overline{\Che \cup \Tse}) = \pi(\emptyset) = \emptyset,
%     \]
%     by the inductive hypothese we may find $\pi' \in \Aut(\A / S \cup \sqrt{\Che})$ such that
%     \[
%         ? \overline{\Che \cup \pi'\pi(\Tse) \cup \Sha} = \emptyset;
%     \]
%     since $\pi' \pi \in \Aut(\A / S \cup \sqrt{\Che})$, we are done.
% \end{Proof}
...
% \begin{Proof}
%     By \autoref{lem:?} we may assume that $?\overline{\Che \cup \Tse \cup \Sha} = \emptyset$ already.
%     As before, fix $\Che, \Sha$ and proceed by induction on the size of $!\overline{\Che \cup \Tse \cup \Sha}$.
%     suppose $(i, a_i) ! (j, b_j)$.
%     Then $(i, a_i) \not\in \overline{\Che}$ and we may assume that $(i, a_i) \in \overline{\Tse}, (j, b_j) \in \overline{\Sha}$ without loss of generality.
%     Moreover, if $(i, a_i) ! (j', b'_{j'})$ in $\overline{\Che \cup \Tse \cup \Sha}$ 
%     then $(j', b'_{j'}) \in \overline{\Sha} \setminus \overline{\Che \cup \Tse}$.
%     Observe that
%     \begin{enumerate}[1)]
%         \item 
%         $b'_{j'} \not\in S$: obvious, as $b' \in \mathcal{O}$;

%         \item 
%         $b'_{j'} \neq a_i$: otherwise $i = j'$, 
%         yet $a_i = b'_{j'}$ are certainly related in the same way as $a_i = a_{j'}$ are related;

%         \item 
%         $b'_{j'} \not\in \sqrt{\Che \cup \Tse}$:
%         otherwise $b'_{j'} = c_k$ for some $c \in \Che \cup \Tse$ so $j' = k$, 
%         implying $(j', b'_{j'}) = (k, c_k) \in \overline{\Che \cup \Tse}$ which is impossible;

%         \item 
%         $a_i \not\in S \cup \sqrt{\Che}$: because $a_i = a'_{i'}$ for $a' \in \Che$ means $(i, a_i) = (i', a'_{i'})$.
%     \end{enumerate}
%     Let $Y$ consist of all such $b'_{j'}$, and put
%     \[
%         X = S \cup \sqrt{\Che \cup \Tse \cup \Sha} \setminus Y \setminus \{a_i\}.
%     \]
%     Then $X, Y, \{a_i\}$ are pairwise disjoint, and $X$ contains $S \cup \sqrt{\Che}$ as well as $\sqrt{\Tse} \setminus \{a_i\}$.
%     So apply the Hammer to find $\tau \in \Aut(\A/X)$ such that $\tau(a_i) \not\in X \cup Y \cup \{a_i\}$ is unrelated to $Y$.
%     We may check that 
%     \[
%         !\overline{\Che \cup \tau(\Tse) \cup \Sha} \subseteq {!\overline{\Che \cup \Tse \cup \Sha}} \setminus \{(i, a_i)\}.
%     \]
%     Indeed, any obstruction on the left must be between $(i', \tau(a'_{i'})) \in \overline{\tau(\Tse)}$ and $(j', b'_{j'}) \in \overline{\Sha}$. 
%     We cannot have $a'_{i'} = a_i$, as $\tau(a'_{i'})$ is unrelated to $b'_{j'} \in Y$ by design.
%     Now if $a'_{i'} \neq a_i$, then $(i', \tau(a'_{i'})) = (i', a'_{i'}) \in \overline{\Tse} \setminus \{(i, a_i)\}$ together with $(j', b'_{j'}) \in \overline{\Sha}$ was already an obstruction on the right.
%     To conclude, apply the inductive hypothesis. 
% \end{Proof}
...
which is furthermore an embedding because $!\overline{A^\pm \cup B^\pm} = \emptyset$ and any $x_i, x'_{i'}$ for $i \neq i'$ are related.
...
Using free amalgamation in $\A_0$ and the generic order in $\A$, we may find some $\pi \in \Aut(\A / X)$ such that $\pi(a_i) \not\in X$, $\pi(a_i) > a_i$, $\pi(a_i) \forkindep a_i$.
We can check that
\begin{enumerate}
    \item 
